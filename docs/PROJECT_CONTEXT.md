...
`LOCAL-FC-05C0` đã hoàn tất audit kiến trúc operational overlays; correction kế tiếp giữ capability chuyên môn và exact scope ở trạng thái chưa quyết định cho đến closure. `LOCAL-FC-05C0D` và ADR-031 nay đã đóng R1-R22, xác lập ba aggregate độc lập, lifecycle/provenance/precedence/make-up semantics và capability/scope tương ứng. Đây chỉ là architecture-ready, không phải implementation: chưa có schema, migration, API hay capability runtime. Candidate tiếp theo là 05C1 persistence foundation, sau đó mới đến 05C2 control plane; Special Activity vẫn deferred theo ADR-031.
