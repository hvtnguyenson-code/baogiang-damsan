...
## 5. Amendment P1-031B — Scheduled-Authority Supersession

### 5.1 Authority and decision

Amendment này ghi nhận quyết định Product Owner đã khóa cho `P1-031B`; hợp đồng đầy đủ được kiểm soát bởi `docs/requirements/P1-031B-OPERATIONAL-START-SCHEDULED-AUTHORITY-SUPERSESSION-ARCHITECTURE.md`. Nó bổ sung ADR-049 theo hướng tiến về phía trước, không viết lại bằng chứng lịch sử P1-030/P1-031.

**PRODUCT OWNER DECISION:** `OPERATIONAL_START` sử dụng một retained lifecycle riêng biệt để supersede một authority đã `PUBLISHED` nhưng chưa bao giờ có hiệu lực.

**CORRECTION IS NOT EXPANDED.** `CORRECTION` tiếp tục chỉ có nghĩa sửa một retained assertion/sự thật lịch sử sai, bắt buộc lý do và `correctsVersionId`. Thay đổi kế hoạch hợp lệ trước hiệu lực không dùng `REVERSED`.

### 5.2 Canonical lifecycle

- Command: `SUPERSEDE_SCHEDULED_AUTHORITY`.
- Eligibility chính: `source.family = OPERATIONAL_START`, `source.status = PUBLISHED`, `source.effectiveUntil = null`, và ngày dân sự HCM do máy chủ sở hữu thỏa `businessCivilDate() < source.effectiveFrom`.
- `businessCivilDate() == source.effectiveFrom` đã quá muộn vì ngày hiệu lực đầu tiên đã bắt đầu.
- Source chuyển sang terminal retained status `SUPERSEDED_BEFORE_EFFECTIVE`; không bị xóa, không đổi payload/effectivity và không bao giờ trở lại authority.
- Successor giữ chính xác `effectiveFrom = source.effectiveFrom`, có `effectiveUntil = null`, và `newOperationalStartDate > businessDate` bên trong unique active AcademicCalendarVersion với `effectiveFrom <= newOperationalStartDate`.
- Lineage dùng trường riêng `supersedesScheduledVersionId`; không dùng lại `replacesVersionId` hoặc `correctsVersionId`.
- Resolver chỉ chọn `PUBLISHED`; terminal source không bao giờ được chọn. Không có latest-row fallback.
- Direct publish guard tiếp tục coi `PUBLISHED`, `REVERSED` và `SUPERSEDED_BEFORE_EFFECTIVE` là bằng chứng stream đã đi vào authoritative lifecycle.

### 5.3 Lifecycle partition

- Scheduled supersession: planned change khi source chưa bao giờ hiệu lực (`businessDate < source.effectiveFrom`).
- Ordinary `REPLACE`: planned prospective change khi source đã bắt đầu hiệu lực nhưng current operational boundary vẫn chưa đến, và interval kế nhiệm muộn hơn vẫn biểu diễn được mà không viết lại elapsed authority.
- `CORRECTION`: erroneous retained assertion/history only; semantics không mở rộng.

### 5.4 Persistence and implementation consequence
