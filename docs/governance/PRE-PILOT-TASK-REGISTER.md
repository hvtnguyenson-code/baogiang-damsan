...
| `P1-031A` Operational-start authority continuity correction | `READY` | `P1-031`, `P1-031B` | All dependencies are CLOSED. The old branch `fix/operational-start-authority-continuity-031a` at remote head `6b7b804a5e82fc54fb280424e82b66d4b48db955` remains bounded pre-architecture evidence only, not canonical implementation and not merge-ready. The implementation task must consume closed P1-031B and deliver both the previously audited finite-authority/replacement-continuity corrections and the scheduled-authority lifecycle: additive migration/DB constraints, status/lineage, guarded HTTP/shared contracts, resolver/read model, server-owned actions, audit/idempotency/concurrency and the complete acceptance matrix | T28, T30 |
