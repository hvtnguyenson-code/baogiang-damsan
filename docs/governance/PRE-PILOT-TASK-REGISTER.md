...
| `P1-012A` Homeroom historical identity read-model correction | `IN_REVIEW` | `P1-012` | Homeroom-capability-scoped bounded historical User identity discovery plus server-owned business-date read model required by P1-013; no new GVCN authority and no `USER_MANAGE` dependency | T13, T14; branch implementation and targeted local non-database gates PASS; isolated PostgreSQL integration NOT RUN / BỊ CHẶN DO MÔI TRƯỜNG; pending independent GitHub review and exact-head CI |
| `P1-013` Homeroom administration workspace | `PLANNED` | `P1-012A` | Bounded admin/PHT UI using frozen backend contracts; no UI-invented authority | T13, T14 |
