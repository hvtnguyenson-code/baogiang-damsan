...
| Task | Status | Depends on | Deliverable / closure | Traceability |
|---|---|---|---|---|
| `P1-031` Operational-start policy implementation | `CLOSED` | `P1-021`, `P1-030`, `P2-003` | Dedicated task branch `feat/operational-start-policy-implementation-031`; starting canonical base `13a87538b38312a2dfb482c358b17ac23f4b2ee8`; final reviewed head `2bf98156f93db47bb986e8e803a137563eadcf18`; independent GitHub review PASS after bounded forward corrections; PR #133 (`feat(policy): implement operational-start authority`); exact-head PR CI #440 (run `34856758210`) SUCCESS; merge/main `a5ee3190171bd5f617a4f32f029328547a0dd37a`; authoritative post-merge main CI #441 (run `34857669684`) SUCCESS on attempt 2 after attempt 1 exposed one unrelated/flaky existing Web auth-flow unit assertion; same merge tree contained no file delta from reviewed PR head and the complete workflow passed on rerun. Delivered scope: `OPERATIONAL_START` production family, v1 strict validator, `ACADEMIC_YEAR` resource, active-calendar validation, lifecycle restrictions (no RETIRE, prospective REPLACE, post-boundary CORRECTION, direct-publish guard), typed resolver, ordinary curricular execution guards, ProgressDebt pre-op no-auto-debt, historical allocator replay retained, live reporting single policy authority, ReportingStatement SNAPSHOT_V2, V1 backward compatibility, pinned operational-start provenance. No schema/migration, no public contract expansion, no Web UI change, no deploy; production remains strictly PRE-OPERATIONAL; CLOSED by `SYNC-P1-031`. A post-closure continuity audit registered P1-031A and P1-031B; downstream P1-032 is blocked until their closure | T28, T30 |
| `P1-031A` Operational-start authority continuity correction | `PLANNED` | `P1-031`, `P1-031B` | Product Owner decision is closed; implementation is accepted but non-startable because P1-031B is not yet CLOSED. Bounded runtime correction remains only on `fix/operational-start-authority-continuity-031a` at reviewed remote head `6b7b804a5e82fc54fb280424e82b66d4b48db955` and is evidence only, not canonical or merged. After P1-031B merge, authoritative post-merge CI and `SYNC-P1-031B`, normal dependency gates may move P1-031A `PLANNED -> READY`; it must then implement the accepted lifecycle together with its already-audited finite-authority and replacement-continuity corrections | T28, T30 |
| `P1-031B` Operational-start scheduled-authority supersession architecture | `IN_REVIEW` | `P1-031` | Docs-only architecture on `docs/operational-start-scheduled-authority-supersession-031b` from canonical `main@f1b160be25045d0f4c661e154ece24c92a3e0fc9`. Records command `SUPERSEDE_SCHEDULED_AUTHORITY`, terminal status `SUPERSEDED_BEFORE_EFFECTIVE`, exact-same-start open successor, dedicated `supersedesScheduledVersionId`, exact guarded HTTP/DTO/result/shared-read contracts, server-owned allowed actions, and OPERATIONAL_START-only database family-scope backstops; `CORRECTION` is not expanded. Future additive P1-031A migration has zero production backfill; resolver, concurrency, audit, idempotency, reporting/frozen provenance and regression contracts are closed without runtime/schema/UI mutation. Pending independent review, merge, authoritative post-merge CI and `SYNC-P1-031B` | T28, T30 |
| `P1-032` Operational-start admin UI integration | `PLANNED` | `P1-022`, `P1-031`, `P1-031A` | Safe business UI for authorized start policy with effectivity/audit visibility. Non-startable until P1-031B is CLOSED, the resulting P1-031A implementation is merged with authoritative post-merge CI, and `SYNC-P1-031A` closes it | T28, T30 |
