...
| `P1-031A` Operational-start authority continuity correction | `BLOCKED_DECISION` | `P1-031`, `P1-031B` | Bounded runtime correction remains only on `fix/operational-start-authority-continuity-031a` at reviewed remote head `6b7b804a5e82fc54fb280424e82b66d4b48db955` and is evidence only, not canonical or merged. It cannot resume until P1-031B architecture is merged, post-merge CI succeeds and `SYNC-P1-031B` closes the architecture task. It must then implement the accepted lifecycle together with its already-audited finite-authority and replacement-continuity corrections | T28, T30 |
| `P1-031B` Operational-start scheduled-authority supersession architecture | `IN_REVIEW` | `P1-031` | Docs-only architecture on `docs/operational-start-scheduled-authority-supersession-031b` from canonical `main@f1b160be25045d0f4c661e154ece24c92a3e0fc9`. Records the explicit Product Owner choice of distinct command `SUPERSEDE_SCHEDULED_AUTHORITY`, terminal retained status `SUPERSEDED_BEFORE_EFFECTIVE`, exact-same-start open successor and dedicated `supersedesScheduledVersionId` for every published source where server-owned HCM `businessDate < source.effectiveFrom`; `CORRECTION` semantics are not expanded. Requires additive future P1-031A status/lineage/terminal-evidence migration with zero production backfill; resolver, concurrency, audit, idempotency, reporting/frozen provenance, UI read and regression contracts are closed without runtime/schema/UI mutation. Pending independent review, merge, authoritative post-merge CI and `SYNC-P1-031B` | T28, T30 |
