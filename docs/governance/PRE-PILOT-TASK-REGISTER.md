...
| `P2-040` Đam San TKB native adapter implementation | `IN_REVIEW` | `P2-030` | Native adapter on top of canonical importer; class/teacher peer cross-check; fail-closed mismatch | T25, T26; implement accepted ADR-047/P2-030 contract; active branch `feat/tkb-native-adapter-040` in review; review rounds 1 & 2 closed (Findings 1–12 corrected with regression tests); Hard Stops A & B resolved by authority reconciliation (ADR-017 for 455 normal rows + structural validation of 120 non-peer slots; ADR-021/026 request-fingerprint provenance); zero schema changes; full unit/native test gates green |
