...
| `P1-031A` Operational-start authority continuity correction | `IN_REVIEW` | `P1-031`, `P1-031B` | Dedicated implementation branch `fix/operational-start-authority-continuity-031a-v2`; canonical starting main `adeade858c2bc7aeac22ee77f8b2a08faf5c438b`. All dependencies are CLOSED. The old branch `fix/operational-start-authority-continuity-031a` at remote head `6b7b804a5e82fc54fb280424e82b66d4b48db955` remains bounded pre-architecture evidence only, not canonical implementation and not merge-ready. Branch implementation now delivers both finite-authority/replacement-continuity corrections and the complete P1-031B scheduled-authority lifecycle: additive zero-backfill migration/DB constraints, retained status/lineage/evidence, guarded exact HTTP/shared contracts, resolver/read serialization, server-owned actions, atomic audit/idempotency/concurrency behavior and A–T regression coverage. Local Prisma validation, contracts/API/Web typecheck, API/Web lint, focused API unit 64/64, Web regression 46/46, reporting provenance regression 51/51, static schema verifier and workflow-contract verifier pass. Isolated PostgreSQL integration is authored but remains for CI/independent review because this workstation exposes no certified `TEST_DATABASE_URL`; no push, PR, merge or deploy has occurred | T28, T30 |
...
| `P1-032` Operational-start admin UI integration | `PLANNED` | `P1-022`, `P1-031`, `P1-031A` | Safe business UI for authorized start policy with effectivity/audit visibility. P1-031B is CLOSED; P1-032 remains non-startable until P1-031A implementation is merged with authoritative post-merge CI and `SYNC-P1-031A` closes it | T28, T30 |
