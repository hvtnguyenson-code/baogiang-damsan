...
| `P1-022` Business Configuration administration workspace | `IN_REVIEW` | `P1-021` | PHT/admin UI for approved business policy only; no access to secrets/TLS/database/process settings; fail-closed adapter registry, empty production registry, typed workflows (create/edit/publish/replace/retire/correct), civil-date resolution lookup, and dedicated navigation | T21, T22; branch `feat/business-configuration-administration-workspace-022` |
