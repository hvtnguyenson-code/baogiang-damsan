...
| `P1-031A` Operational-start authority continuity correction | `IN_REVIEW` | `P1-031` | Correct the post-closure lifecycle defect where an initial `OPERATIONAL_START` draft/publication may retain finite `effectiveUntil` even though RETIRE is forbidden and REPLACE accepts only an open-ended source, creating an unrecoverable policy gap. Enforce open-ended initial authority at create/publish defense-in-depth gates; audit edit/publish/replace/retire/correct/resolver/database/downstream assumptions; require replacement effectivity no later than both current and new operational-start dates while retaining the server-owned future-effectivity rule; preserve CORRECTION effectivity and fail-closed resolution. Dedicated branch `fix/operational-start-authority-continuity-031a`; canonical start `f1b160be25045d0f4c661e154ece24c92a3e0fc9`; local API lint/typecheck/static schema and 74-suite/1,243-test API unit regression PASS; database integration regressions added but local execution requires an explicitly certified isolated test database; no schema/migration, UI, deploy or production mutation | T28, T30 |
| `P1-032` Operational-start admin UI integration | `PLANNED` | `P1-022`, `P1-031`, `P1-031A` | Safe business UI for authorized start policy with effectivity/audit visibility. Blocked until P1-031A implementation merge, authoritative post-merge CI and `SYNC-P1-031A` closure | T28, T30 |
