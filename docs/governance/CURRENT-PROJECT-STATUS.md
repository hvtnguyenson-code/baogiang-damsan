...
Active branch: `feat/tkb-native-adapter-040`, based on canonical `origin/main@54f5a61b2045e866cd566290cf38c6273e7c28ff`. Traceability: T25, T26. Following independent ChatGPT GitHub review of head `994a666`, Findings 1–6 were corrected with regression tests: composite `TeacherSourceRowRef` keying prevents cross-sheet rowNumber collapse; single canonical native provenance sentinel `ALL_SHEETS` unifies confirmation and replay verification; workbook effective date is reconciled against target week; structural teacher/class headers are validated with fail-closed semantics; afternoon class headers are validated; and business/header cell safety (formulas, hyperlinks, invalid merged cells) is enforced. Read-only architectural audits for Hard Stop A (120 special non-peer slots representation) and Hard Stop B (raw workbook SHA receipt provenance) were completed. 47 native tests + full test gates pass with zero schema modifications. P2-040 remains `IN_REVIEW` pending architecture decisions on Hard Stops A and B; P2-050 remains `PLANNED` and unimplemented.
