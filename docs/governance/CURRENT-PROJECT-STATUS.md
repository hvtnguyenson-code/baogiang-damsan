...
- `P1-031C` — Operational-start Academic-Year options read-model enablement — **READY**. All dependencies (`P1-021`, `P1-031`, `P1-031A`) are CLOSED. Smallest Business Configuration-owned AcademicYear options read model (`GET /api/business-configuration/academic-year-options`) under `BUSINESS_CONFIGURATION_MANAGE / SCHOOL_WIDE` required by P1-032 resource picker; implementation assigned to CODEX under AGENTS.md.
...
- unlocks downstream: `P1-031C` (`READY`), enabling Business Configuration AcademicYear options before `P1-032` (`PLANNED`).
...
- unlocks `P1-031A` to `READY` (now `CLOSED` by `SYNC-P1-031A`); `P1-031C` is registered as `READY`; `P1-032` is `PLANNED` pending `P1-031C` closure.
...
- initially unlocked downstream P1-032; the post-closure continuity audit registered P1-031A/P1-031B (both CLOSED); P1-032 preflight then established the AcademicYear options read seam P1-031C (now READY); P1-032 is returned to PLANNED pending P1-031C closure.
...
1. `P1-031C` — Operational-start Academic-Year options read-model enablement (`READY`; all dependencies `P1-021`, `P1-031`, `P1-031A` are CLOSED).
