...
| T13 | HĐTN class-level activity uses effective homeroom responsibility | LOCAL-FC-05A0 confirms class HĐTN uses effective homeroom teacher; ADR-010/012 had deferred the canonical domain | ADR-045 accepted by P1-010: separate AcademicYear-owned, class/date-effective HomeroomAssignment with exact fail-closed historical resolver, current-vs-historical eligibility distinction and dedicated management authority | P1-011 persistence, P1-012 control plane and closed P1-012A read-model correction now provide complete Homeroom backend authority, including bounded historical identity discovery without `USER_MANAGE` and server-owned `businessDate`; strict current eligible-teacher semantics remain unchanged | `RESTORE` | `P1-013` adds UI only; P4 may consume completed Homeroom authority subject to its own dependencies |
| T14 | Historical homeroom responsibility must not drift after teacher or current account-state change | historical-retention rules from ADR-010/012/038 + P0 baseline | ADR-045 accepted: retained effective intervals, bounded historical truth survives later User/profile changes, frozen downstream GVCN provenance and explicit correction semantics | P1-011/P1-012 retain and resolve historical truth independently of current account/profile state; closed P1-012A exposes exact inactive/non-teaching/profile-less User identity discovery and the server-owned business date without weakening current/future eligibility or requiring generic user authority | `KEEP` | `P1-013` adds admin UI only; no backend historical-authority re-entry required |
