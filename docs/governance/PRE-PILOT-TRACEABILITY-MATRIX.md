...
| T13 | HĐTN class-level activity uses effective homeroom responsibility | LOCAL-FC-05A0 confirms class HĐTN uses effective homeroom teacher; ADR-010/012 had deferred the canonical domain | ADR-045 accepted by P1-010: separate AcademicYear-owned, class/date-effective HomeroomAssignment with exact fail-closed historical resolver, current-vs-historical eligibility distinction and dedicated management authority | P1-011 retains Prisma/PostgreSQL history; P1-012 branch implements the dedicated SCHOOL_WIDE capability, create/change/end/correct/read/options endpoints, exact fail-closed resolver and calendar compatibility, pending independent review and exact-head CI | `RESTORE` | `P1-012` control plane; `P4-010` consumes accepted ADR-045 authority |
| T14 | Historical homeroom responsibility must not drift after teacher or current account-state change | historical-retention rules from ADR-010/012/038 + P0 baseline | ADR-045 accepted: retained effective intervals, bounded historical truth survives later User/profile changes, frozen downstream GVCN provenance and explicit correction semantics | P1-011 retains ACTIVE/REVERSED evidence and DB exclusion; P1-012 branch implements historical eligibility, inclusive change, 1..N retained correction lineage, same-transaction audits and RESOLVED/MISSING/AMBIGUOUS/CORRUPT handling, pending independent review and exact-head CI | `RESTORE` | `P1-012`; `P1-013` adds admin UI only |
