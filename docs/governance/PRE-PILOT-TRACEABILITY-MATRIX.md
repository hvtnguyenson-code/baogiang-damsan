...
| T13 | HĐTN class-level activity uses effective homeroom responsibility | LOCAL-FC-05A0 confirms class HĐTN uses effective homeroom teacher; ADR-010/012 had deferred the canonical domain | ADR-045 accepted by P1-010: separate AcademicYear-owned, class/date-effective HomeroomAssignment with exact fail-closed historical resolver, current-vs-historical eligibility distinction and dedicated management authority | P1-011 persistence and closed P1-012 control plane provide the canonical authority; P1-012A branch adds bounded Homeroom-scoped historical identity discovery and the server business date without new GVCN semantics, pending independent review and exact-head CI | `RESTORE` | `P1-012A` read-model correction -> `P1-013` UI; P4 may consume completed Homeroom authority subject to its own dependencies |
| T14 | Historical homeroom responsibility must not drift after teacher or current account-state change | historical-retention rules from ADR-010/012/038 + P0 baseline | ADR-045 accepted: retained effective intervals, bounded historical truth survives later User/profile changes, frozen downstream GVCN provenance and explicit correction semantics | P1-011/P1-012 retain and resolve historical truth independently of current account/profile state; P1-012A branch adds bounded discovery of exact inactive/non-teaching/profile-less User identities plus the server-owned business date so P1-013 need not weaken eligibility or use generic user authority, pending independent review and exact-head CI | `KEEP` | `P1-012A` read-model correction -> `P1-013` admin UI; no change to backend historical authority |
