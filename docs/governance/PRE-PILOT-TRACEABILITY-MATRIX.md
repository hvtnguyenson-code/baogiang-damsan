...
| T13 | HĐTN class-level activity uses effective homeroom responsibility | LOCAL-FC-05A0 confirms class HĐTN uses effective homeroom teacher; ADR-010/012 had deferred the canonical domain | ADR-045 accepted by P1-010: separate AcademicYear-owned, class/date-effective HomeroomAssignment with exact fail-closed historical resolver, current-vs-historical eligibility distinction and dedicated management authority | Architecture + retained Prisma/PostgreSQL persistence implemented by P1-011; management capability/control plane still absent | `RESTORE` | `P1-012` control plane; `P4-010` consumes accepted ADR-045 authority |
| T14 | Historical homeroom responsibility must not drift after teacher or current account-state change | historical-retention rules from ADR-010/012/038 + P0 baseline | ADR-045 accepted: retained effective intervals, bounded historical truth survives later User/profile changes, frozen downstream GVCN provenance and explicit correction semantics | P1-011 persistence now retains ACTIVE/REVERSED history, exact teacher/actor identities, correction lineage and DB-level same-class/date current-truth invariants; command/control plane still absent | `RESTORE` | `P1-012`; `P1-013` adds admin UI only |
