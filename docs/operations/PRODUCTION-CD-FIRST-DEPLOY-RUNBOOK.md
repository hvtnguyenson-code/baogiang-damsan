...
4. Bootstrap the Scheduled Task runtime contract P0-3: an immutable startup runtime bundle containing both `start-baogiang-api.ps1` and its sibling `deployment-common.ps1` from the exact reviewed GitHub commit; both absolute shared paths and SHA-256 values recorded in the marker; and the exact Scheduled Task action bound to the hash-verified wrapper. The task must contain exactly one enabled `AtStartup`/Boot trigger (`MSFT_TaskBootTrigger`) and no other trigger. Bootstrap leaves the whole task **Disabled** and the runtime stopped; the Boot trigger remains in its definition, but cannot run while the task is disabled. Do not assume `current` exists. Windows Service production bootstrap is deferred until reviewed Service architecture exists.
