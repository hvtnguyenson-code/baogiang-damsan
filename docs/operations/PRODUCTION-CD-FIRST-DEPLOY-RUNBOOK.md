...
1. Operator and independent reviewer confirm the exact non-secret `DeploymentIdentity`, `ApiRuntimeIdentity`, and `WebRuntimeIdentity`. Each input is an exact SID or an account name resolvable to a SID on the host; do not place a production account name in repository documentation.
2. Run `production-root-acl-plan.ps1` with the reviewed root, identities, EnvFile, StartupWrapper and an existing operator-owned report directory. Review the JSON desired state for the protected root, exact six subdirectories and sensitive leaves. The plan is deterministic and does not create a root/directory or mutate an ACL.
3. Under separate explicit operational approval, the operator manually creates only the dedicated root and required subdirectories, places the separately reviewed marker/env/startup-bundle leaves at their already approved paths, and manually applies the planned ACLs. This ACL slice does not define startup-bundle provenance/install/update; that remains a separate P1 gate. Do not use a drive root, `Windows`, `System32`, `Program Files`, Nginx root, DamSanV5 root, or boarding-management root. There is no automatic remediation or hidden apply mode.
4. Run `production-root-acl-verify.ps1` with the same inputs. Stop unless the report state is `PASS`: a missing/type-mismatched/reparse root or required direct child, unprotected DACL, missing/unexpected/inherited/broad ACE, explicit DENY, wrong numeric rights or wrong inheritance flags is a failure. The verifier is read-only apart from writing its report. It does not require `current`; root inheritance supplies future direct-child traverse/read/modify authority. It verifies exact DACL shape, not owner policy or all effective authorization expanded through nested local/domain group membership, which remains reviewed operator evidence.
5. Only after the ACL verifier passes, continue marker bootstrap evidence for the value-free file at `shared\deployment-identity.json`. Use exact `schemaVersion: 1` marker shape from `PRODUCTION-ENVIRONMENT-CONFIGURATION.md`; unknown/missing/empty/wrong-type fields are rejected. Verify its canonical root, domain, API port, executable paths, env path, wrapper, entry point and exact Scheduled Task action.
6. Continue server-side production environment evidence with `validate-production-environment.ps1`, not the startup wrapper: `current` is not required and Node is not started. Retain/send only its value-free PASS result for review; never send the env file or values to ChatGPT, Codex, or GitHub. Do not store bootstrap-admin or test-database variables in the runtime file.
7. Continue startup-bundle and Scheduled Task runtime evidence under the P0-3 contract: the immutable bundle contains both `start-baogiang-api.ps1` and its sibling `deployment-common.ps1` from the exact reviewed GitHub commit; both absolute shared paths and SHA-256 values are recorded in the marker; and the exact Scheduled Task action is bound to the hash-verified wrapper. The task must contain exactly one enabled `AtStartup`/Boot trigger (`MSFT_TaskBootTrigger`) and no other trigger. Bootstrap leaves the whole task **Disabled** and the runtime stopped; the Boot trigger remains in its definition, but cannot run while the task is disabled. Do not assume `current` exists. Windows Service production bootstrap is deferred until reviewed Service architecture exists.
8. Configure/verify the dedicated Nginx block: HTTPS domain, SPA fallback, `/api` proxy to `127.0.0.1:3100`, forwarded headers and reviewed request limits. Run the exact `nginx -t -c <reviewed-config>` before a targeted reload. Never stop/restart all Nginx.

Stop for independent review of the ACL plan/verifier reports, marker, wrapper action, env validation, Nginx test and isolation evidence. Neither ACL tool changes filesystem security; every ACL correction remains a separately approved manual operator action.
