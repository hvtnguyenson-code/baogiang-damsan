...
Nginx plan, verifier, and deployment syntax testing bind the exact triple `nginxExe`, `foreignIsolation.reviewedNginxPrefix`, and `nginxConfig`/`foreignIsolation.reviewedNginxConfig`. Marker schema remains version 1. The exact syntax vector is `<nginxExe> -p <reviewedNginxPrefix> -t -c <reviewedNginxConfig>`; reload is never automatic and exists only as a reviewed manual command vector in the Nginx plan.
