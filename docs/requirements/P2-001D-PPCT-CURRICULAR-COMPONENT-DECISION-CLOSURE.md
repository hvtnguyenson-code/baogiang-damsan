...
2. **Quyền sở hữu Thành phần Bất biến:** 
   - `PpctItem` sở hữu trực tiếp thuộc tính `component`.
...
3. **Tọa độ Quan hệ Bản sao trên Revision:**
   ...
   - Tầng cơ sở dữ liệu (`P2-002`) bắt buộc thiết lập composite foreign key:
     ...
     bảo đảm `revision.component == item.component`, loại trừ hoàn toàn rủi ro trôi lệch thành phần.
4. **Bảo tồn Kế hoạch Khung Dùng chung:**
   - Giữ nguyên thực thể `PpctPlan = AcademicYear + Subject + Grade`.
...
1. **Ràng buộc Duy nhất Mới:**
   Thay thế `@@unique([versionId, sequence])` bằng:
   ```prisma
...
## 5. Hiệu lực Khoảng ngày và Quy tắc Biên Tuần học

1. **Một Bảng Liên kết Duy nhất:**
   - `PpctClassAssociation` tiếp tục là bảng duy nhất quản lý liên kết phiên bản và hồ sơ áp dụng theo khoảng ngày dân sự GiST (`effectiveRange`). Không tạo bảng riêng cho chuyên đề.
2. **Đồng nhất trên Toàn bộ Tuần học (AcademicWeek):**
   - Hồ sơ `curricularProfile` bắt buộc phải giữ nguyên một giá trị duy nhất trong phạm vi từng `AcademicWeek` chuẩn tắc.
3. **Chặn Lỗi Phân tách Tuần (Fail-Closed):**
   - Nếu dữ liệu liên kết tạo ra sự thay đổi `curricularProfile` giữa tuần:
...
1. **Tập Ứng viên Chuẩn tắc:**
   - Được xác định theo bộ khóa: `(academicYearId, schoolClassId, subjectId, academicWeekId)`.
   - Thu thập toàn bộ các cơ hội TKB bình thường (`NORMAL_TIMETABLE_OPPORTUNITY`) có ngày dân sự nằm trong tuần học đó. Hỗ trợ đầy đủ việc chuyển giao TKB giữa tuần (các cơ hội có thể đến từ các `TimetableVersion` khác nhau có hiệu lực theo ngày).
...
## 7. Quy tắc Tuần Dị thường và Phân loại Kế hoạch vs. Vận hành

1. **Tuần có 0 cơ hội:** Không định tuyến, không phân bổ, không tạo cơ hội giả tạo, không tiêu thụ PPCT, không tạo nợ.
2. **Tuần có đúng 1 cơ hội đối với lớp `CORE_PLUS_SPECIALIZED_STUDY`:**
...
3. **Bất biến Lập kế hoạch vs. Vận hành:**
   - Định tuyến thành phần là phân loại lập kế hoạch (planning semantics), được xác lập trước khi áp dụng các kết quả vận hành.
   - Các sự kiện vận hành (nghỉ lễ, hủy tiết, dạy thay, vắng mặt) **KHÔNG BAO GIỜ** làm thay đổi thành phần kế hoạch của cơ hội.
...
2. **Profile Chuẩn tắc Mới:** `NORMAL_BASE_PPCT_COMPONENT_V2` (do `P2-003` triển khai):
   ...
   - Đánh giá dung lượng tuần: lớp bật chuyên đề bắt buộc phải có ít nhất 2 cơ hội trong mỗi tuần có tiết.
...
## 10. Hợp đồng Phả hệ Bài học (Lineage Contract)

1. **Cấm Tuyệt đối Lineage Vượt Thành phần:**
   - Mọi quan hệ `CARRY_FORWARD`, `SPLIT (1 -> N)`, `MERGE (N -> 1)` bắt buộc phải diễn ra trong cùng một thành phần:
     `predecessor.component == successor.component`
   - Vi phạm sẽ bị từ chối với mã lỗi ngữ nghĩa:
...
2. **Mục tiêu Ràng buộc DB của P2-002:**
   ...
   - Composite FK bảo vệ `revision.component == item.component`.
   - Khóa duy nhất: `@@unique([versionId, component, sequence])`.
...
- **`P2-002`**: Chịu trách nhiệm triển khai schema migration, composite constraints, control plane soạn thảo bản nháp, xuất bản nguyên tử và quản lý `curricularProfile` trên liên kết lớp.
- **`P2-003`**: Chịu trách nhiệm triển khai runtime định tuyến tuần, hai con trỏ tiến độ độc lập, profile sẵn sàng `V2`, và chiếu nợ tiến độ.
