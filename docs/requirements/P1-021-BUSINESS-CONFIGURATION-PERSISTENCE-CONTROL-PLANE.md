...
Lifecycle mutation dùng Serializable transaction, optimistic draft revision, receipt idempotency và same-transaction `AuditService`. Migration thêm resource/lifecycle checks, partial unique streams, inclusive GIST exclusion cho PUBLISHED overlap, lineage trigger và immutable-published trigger.
...
| 17 | Transaction-safe / idempotent concurrency | `apps/api/src/business-configuration/business-configuration.service.ts` (`mutate()` using Serializable transaction, `canonicalJson()`, `fingerprint()`), `migration.sql` (`business_policy_commands_actor_command_key`) | `scripts/ci/verify-business-configuration-schema.sql` (Invariants 30, 31, 32), `apps/api/test/business-configuration/business-configuration.integration.spec.ts` (Sections 18, 22, 23) | PASS (static/unit), LOCAL NOT RUN — thiếu isolated PostgreSQL environment (migration SQL/integration) |
