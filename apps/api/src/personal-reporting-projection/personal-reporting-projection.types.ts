...
export const PERSONAL_TEACHING_REPORTING_PROJECTION_PROFILE =
  "PERSONAL_TEACHING_REPORTING_PROJECTION_V1" as const;
export const PERSONAL_REPORTING_CLOCK = Symbol("PERSONAL_REPORTING_CLOCK");
export interface PersonalReportingClock {
  now(): Date;
}
export interface ResolvePersonalReportingProjectionInput {
...
export interface PersonalReportingFinding {
  severity: "BLOCKER";
  code:
    | ReportingFinding["code"]
...
export interface PersonalReportingSection {
  schoolClassId: string;
  subjectId: string;
  ...
  findings: PersonalReportingFinding[];
}
export interface PersonalReportingProjection {
  profile: typeof PERSONAL_TEACHING_REPORTING_PROJECTION_PROFILE;
  scope: ResolvePersonalReportingProjectionInput;
  responsibilityState: "RESPONSIBILITY_PRESENT" | "ZERO_RESPONSIBILITY";
  status: "PASS" | "BLOCKED";
  counts: ReportingCounts | null;
  responsibilityManifest: PersonalResponsibilityInterval[];
  sections: PersonalReportingSection[];
  findings: PersonalReportingFinding[];
  evaluatedAt: string;
}
