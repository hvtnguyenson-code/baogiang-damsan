...
/* Retained relation payloads are deliberately kept local to this internal read composer. */
/* eslint-disable @typescript-eslint/no-explicit-any */
import { OperationalOverlayStatus, PpctVersionStatus, Prisma, SpecialActivityStatus, TimetableVersionStatus } from '@prisma/client';
...
export class ResolvedLessonOccurrencesService {
  ...
  private async resolveSnapshot(tx: Prisma.TransactionClient, input: ResolveLessonOccurrencesInput): Promise<ResolvedLessonOccurrencesResult> {
    ...
    const [versions, activities, makeups] = await Promise.all([
      ...
      tx.specialActivity.findMany({ where: { academicYearId: input.academicYearId, civilDate: date, status: SpecialActivityStatus.ACTIVE }, include: { classTargets: true, timeSlots: { include: { timeSlotDefinition: true } }, staffing: true }, orderBy: { id: 'asc' } }),
      tx.makeupTeachingSchedule.findMany({ where: { academicYearId: input.academicYearId, targetCivilDate: date, status: OperationalOverlayStatus.ACTIVE }, include: { targetTimeSlotDefinition: true }, orderBy: { id: 'asc' } }),
    ]);
  ...
  private async resolveNormal(tx: Prisma.TransactionClient, input: ResolveLessonOccurrencesInput, date: Date, version: any, activities: any[], output: NormalStructuralOccurrence[], blocker: (code: StructuralOccurrenceFinding['code'], occurrenceKey: string | null, ...ids: string[]) => void): Promise<void> {
    ...
    const [entries, interruptions, exceptions, dispositions] = await Promise.all([
      tx.timetableEntry.findMany({ where: { timetableVersionId: version.id, weekday: weekdayFor(date) as any }, include: { timeSlotDefinition: true, schoolClass: true, teachingAssignment: true }, orderBy: { id: 'asc' } }),
    ...
    for (const entry of entries) {
      ...
      const suppressionIds = activities.filter((a) => a.classTargets.some((x: any) => x.schoolClassId === entry.schoolClassId) && a.timeSlots.some((x: any) => intervalsOverlap(x.timeSlotDefinition.startTime, x.timeSlotDefinition.endTime, entry.timeSlotDefinition.startTime, entry.timeSlotDefinition.endTime))).map((x) => x.id).sort();
  ...
  private detectActivityCollisions(activities: any[], makeups: any[], blocker: (code: StructuralOccurrenceFinding['code'], occurrenceKey: string | null, ...ids: string[]) => void): void {
    for (const activity of activities) for (const makeup of makeups) if (activity.timeSlots.some((s: any) => intervalsOverlap(s.timeSlotDefinition.startTime, s.timeSlotDefinition.endTime, makeup.targetTimeSlotDefinition.startTime, makeup.targetTimeSlotDefinition.endTime)) && (activity.classTargets.some((x: any) => x.schoolClassId === makeup.schoolClassId) || activity.staffing.some((x: any) => x.scheduledTeacherUserId === makeup.scheduledTeacherUserId))) blocker('ACTIVE_SPECIAL_ACTIVITY_MAKEUP_COLLISION', `MAKEUP:${makeup.id}`, activity.id, makeup.id);
    for (let i = 0; i < activities.length; i += 1) for (let j = i + 1; j < activities.length; j += 1) { const a = activities[i]!, b = activities[j]!; if (a.timeSlots.some((x: any) => b.timeSlots.some((y: any) => intervalsOverlap(x.timeSlotDefinition.startTime, x.timeSlotDefinition.endTime, y.timeSlotDefinition.startTime, y.timeSlotDefinition.endTime))) && (a.classTargets.some((x: any) => b.classTargets.some((y: any) => y.schoolClassId === x.schoolClassId)) || a.staffing.some((x: any) => b.staffing.some((y: any) => y.scheduledTeacherUserId === x.scheduledTeacherUserId)))) blocker('ACTIVE_SPECIAL_ACTIVITY_COLLISION', null, a.id, b.id); }
  }
}
