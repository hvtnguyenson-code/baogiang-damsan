...
import { ProgressDebtItemV2, ProgressDebtOperationalStartAuthority } from '../progress-debt/progress-debt.types';
import {
  ReportingCounts,
  ReportingDetail,
  ReportingFinding,
  ...
  ResolveReportingProjectionInput,
  TEACHING_REPORTING_PROJECTION_PROFILE,
} from './reporting-projection.types';

export interface ReportingProjectionContext {
  operationalStartPolicy?: ProgressDebtOperationalStartAuthority;
  policyResolutionCivilDate?: CivilDateString;
}
...
export class ReportingProjectionService {
  ...
  async resolve(
    input: ResolveReportingProjectionInput,
    context?: ReportingProjectionContext,
  ): Promise<ReportingProjection> {
    this.validateInput(input);
    const policyResolutionCivilDate =
      context?.policyResolutionCivilDate ??
      (this.businessConfiguration.businessCivilDate() as CivilDateString);
    return this.prisma.$transaction(
      (tx) =>
        this.resolveInTransaction(tx, input, {
          ...context,
  ...
  async resolveInTransaction(
    ...
    const operationalStartAuthority: ProgressDebtOperationalStartAuthority =
      context?.operationalStartPolicy ??
      (await (async () => {
        const policy =
          await this.businessConfiguration.resolveOperationalStartPolicy(
    ...
    if (
      classes.length !== ids.length ||
      classes.some((row) => row.academicYearId !== input.academicYearId)
    ) {
      throw new BadRequestException(
        'Every reporting root school class must belong to academicYearId.',
      );
    }
    const roots: ReportingRootProjection[] = [];
    for (const root of [...input.roots].sort((a, b) =>
      this.rootKey(a).localeCompare(this.rootKey(b)),
    )) {
      const upstream = await this.progressDebt.resolveInTransactionV2(
        tx,
        {
          academicYearId: input.academicYearId,
          schoolClassId: root.schoolClassId,
          subjectId: root.subjectId,
          asOfInstant: input.asOfInstant,
        },
        operationalStartAuthority,
      );
      ...
      const items = upstream.items.filter((item) => {
        const date = parseCivilDate(item.sourceCivilDate);
        return date >= from && date <= to;
      });
      const slots = await tx.timeSlotDefinition.findMany({
        where: {
          id: {
            in: [...new Set(items.map((item) => item.sourceTimeSlotDefinitionId))],
          },
        },
      ...
      const missing = items.filter(
        (item) => !byId.has(item.sourceTimeSlotDefinitionId),
      );
      if (missing.length) {
        roots.push({
          scope: root,
          ...
          findings: missing
            .map(
              (item): ReportingFinding => ({
                severity: 'BLOCKER',
                code: 'SOURCE_TIME_SLOT_PROVENANCE_MISSING',
                reason:
                  'Required retained source time-slot provenance cannot be resolved.',
                entityIds: [item.sourceTimeSlotDefinitionId],
                occurrenceKey: item.sourceNormalOccurrenceKey,
              }),
            )
      ...
      const details = items
        .map((item) =>
          this.detail(
            ...
            root,
            item,
            byId.get(item.sourceTimeSlotDefinitionId)!,
          ),
        )
    ...
    return {
      profile: TEACHING_REPORTING_PROJECTION_PROFILE,
      scope: input,
      status: blocked ? 'BLOCKED' : 'PASS',
      counts: blocked ? null : this.sum(roots),
      roots,
  ...
  private count(details: ReportingDetail[]): ReportingCounts {
    const completed = details.filter((x) => x.classification === 'COMPLETED').length;
    const debt = details.filter((x) => x.classification === 'PROVEN_OPEN_DEBT').length;
    const gap = details.length - completed - debt;
    return { distributedElapsedCount: details.length, completedCount: completed, openDebtCount: debt, lateCount: debt, unconfirmedGapCount: gap };
  }

  private sum(roots: ReportingRootProjection[]): ReportingCounts {
    return roots.reduce((total, root) => {
      const count = root.counts!;
      return { distributedElapsedCount: total.distributedElapsedCount + count.distributedElapsedCount, completedCount: total.completedCount + count.completedCount, openDebtCount: total.openDebtCount + count.openDebtCount, lateCount: total.lateCount + count.lateCount, unconfirmedGapCount: total.unconfirmedGapCount + count.unconfirmedGapCount };
    }, { distributedElapsedCount: 0, completedCount: 0, openDebtCount: 0, lateCount: 0, unconfirmedGapCount: 0 });
  }

  private compare(a: ReportingDetail, b: ReportingDetail): number {
    return a.sourceCivilDate.localeCompare(b.sourceCivilDate) || a.sourceSlotStart.localeCompare(b.sourceSlotStart) || a.sourceSlotEnd.localeCompare(b.sourceSlotEnd) || a.sourceNormalOccurrenceKey.localeCompare(b.sourceNormalOccurrenceKey);
  }

  private rootKey(root: ReportingRootInput): string {
    return root.schoolClassId + ':' + root.subjectId;
  }
}