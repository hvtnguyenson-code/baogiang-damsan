...
import { ChangeHomeroomTeacherDto, CorrectHomeroomAssignmentDto, CreateHomeroomAssignmentDto, EndHomeroomAssignmentDto, HomeroomEligibleTeachersDto, HomeroomPageDto, ListHomeroomAssignmentsDto } from './dto';
...
export class HomeroomAssignmentsController {
  ...
  @Get('academic-years/:academicYearId/homeroom-assignments/resolve') @UseGuards(SessionAuthGuard, CapabilityGuard) resolve(@Param('academicYearId', ParseUUIDPipe) year: string, @Query('schoolClassId', ParseUUIDPipe) schoolClassId: string, @Query('on') on: string) { return this.service.resolve(year, schoolClassId, on); }
