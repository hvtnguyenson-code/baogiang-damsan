...
import { HeaderMapping, locateHeader } from './workbook-inspection';
import { MAX_MAPPED_CELL_TEXT_LENGTH } from './workbook-limits';
import { ParsedWorkbook, ParsedWorkbookCell } from './workbook-parser.types';

export type RevisionContext = Prisma.TimetableImportProfileRevisionGetPayload<{
  include: { profile: true; columnMappings: true };
}>;

const messages: Record<string, string> = {
  HIDDEN_MAPPED_DATA: 'Hidden row or column contains mapped timetable data.',
  NONBLANK_ROW_WITHOUT_MAPPED_DATA: 'Nonblank row contains no mapped data.',
  PARTIALLY_BLANK_MAPPED_ROW: 'Mapped row is incomplete.',
  FORMULA_IN_MAPPED_CELL: 'Formula is not allowed in a mapped cell.',
  HYPERLINK_IN_MAPPED_CELL: 'Hyperlink is not allowed in a mapped cell.',
  MERGED_MAPPED_CELL: 'Merged mapped cell is not allowed.',
  UNSUPPORTED_MAPPED_CELL_TYPE: 'Mapped cell type is unsupported.',
  MAPPED_VALUE_TOO_LONG: 'Mapped value is too long.',
  INVALID_WEEKDAY: 'Weekday token is invalid.',
  INVALID_SESSION: 'Session token is invalid.',
  INVALID_PERIOD_ORDINAL: 'Period ordinal is invalid.',
  WEEKDAY_NOT_IN_CALENDAR: 'Weekday is not configured for the calendar.',
  SLOT_NOT_FOUND: 'Time slot coordinate was not found.',
  SLOT_NOT_ACTIVE: 'Time slot is inactive.',
  SLOT_NOT_REGULAR_TEACHING: 'Time slot does not allow regular teaching.',
  CLASS_NOT_FOUND: 'Class was not found.',
  CLASS_INACTIVE: 'Class is inactive.',
  CLASS_IDENTITY_CONFLICT: 'Class code and alias disagree.',
  SUBJECT_NOT_FOUND: 'Subject was not found.',
  SUBJECT_INACTIVE: 'Subject is inactive.',
  SUBJECT_IDENTITY_CONFLICT: 'Subject code and alias disagree.',
  TEACHER_NOT_FOUND: 'Teacher was not found.',
  TEACHER_AMBIGUOUS: 'Teacher identifier is ambiguous.',
  TEACHER_INACTIVE: 'Teacher is inactive.',
  TEACHER_NOT_TEACHING_STAFF: 'User is not teaching staff.',
  ASSIGNMENT_NOT_FOUND: 'Teaching assignment was not found.',
  ASSIGNMENT_AMBIGUOUS: 'Teaching assignment is ambiguous.',
  ASSIGNMENT_COVERAGE_GAP: 'Teaching assignment does not cover the validation envelope.',
...
@Injectable()
export class WorkbookCanonicalizationService {
  constructor(private readonly prisma: PrismaService) {}

  async requireActiveRevision(
    id: string,
    db: Prisma.TransactionClient = this.prisma,
  ): Promise<RevisionContext> {
    const row = await db.timetableImportProfileRevision.findUnique({
      where: { id },
      include: { profile: true, columnMappings: true },
    });
    if (!row) throw new NotFoundException('Không tìm thấy phiên bản cấu hình nhập.');
    if (!row.isActive) {
    ...
    this.assertUniqueMappings(row);
    return row;
  }

  headerMappings(revision: RevisionContext): HeaderMapping[] {
    const order = new Map(semanticFieldOrder.map((field, index) => [field, index]));
    return [...revision.columnMappings]
      .sort((a, b) => (order.get(a.semanticField) ?? 99) - (order.get(b.semanticField) ?? 99))
      .map((item) => ({ semanticField: item.semanticField, sourceHeaderKey: item.sourceHeaderKey }));
  }
  ...
  async preview(
    ...
    if (!sheet || sheet.state !== 'VISIBLE' || !sheet.rows.some((row) => row.cells.some((cell) => cell.kind !== 'BLANK'))) {
      throw new BadRequestException({
        error: 'TIMETABLE_IMPORT_SHEET_NOT_SELECTABLE',
    ...
    if (!located?.candidate.complete) {
      throw new BadRequestException({
        ...
        message: 'Confirmed header row does not contain the exact mapping.',
      });
    }
    ...
    for (const sourceRow of sheet.rows.filter((row) => row.number > dto.headerRowNumber)) {
      ...
      const mapped = Object.fromEntries(
        semanticFields.map((field) => [field, sourceRow.cells[located.columns[field] - 1]!]),
      ) as Record<TimetableImportSemanticField, ParsedWorkbookCell>;
      const present = semanticFields.filter((field) => mapped[field].kind !== 'BLANK');
      if (present.length === 0) {
        issues.push(this.issue('NONBLANK_ROW_WITHOUT_MAPPED_DATA', sourceRow.number));
        continue;
      }
      if (present.length !== semanticFields.length) {
        issues.push(this.issue('PARTIALLY_BLANK_MAPPED_ROW', sourceRow.number));
        continue;
      }
      ...
      for (const field of semanticFields) {
        ...
        const code = cell.formula ? 'FORMULA_IN_MAPPED_CELL'
          : cell.hyperlink ? 'HYPERLINK_IN_MAPPED_CELL'
            : cell.merged ? 'MERGED_MAPPED_CELL'
              : cell.kind !== 'TEXT' && !(permitsNumber && cell.kind === 'NUMBER')
                ? 'UNSUPPORTED_MAPPED_CELL_TYPE' : undefined;
        if (code) {
          issues.push(this.issue(code, sourceRow.number, field));
          unsafe = true;
          continue;
        }
        const value = normalizeHumanText(cell.text!);
        if (cell.textOverLimit || value.length > MAX_MAPPED_CELL_TEXT_LENGTH) {
          issues.push(this.issue('MAPPED_VALUE_TOO_LONG', sourceRow.number, field, value));
          unsafe = true;
        }
        values[field] = value;
        if ((sourceRow.hidden || sheet.hiddenColumns.includes(located.columns[field])) && value) {
          issues.push(this.issue('HIDDEN_MAPPED_DATA', sourceRow.number, field));
        }
      }
      if (unsafe) continue;
      const weekday = parseWeekday(values.WEEKDAY);
      const session = parseSession(values.SESSION);
      const ordinal = parsePeriodOrdinal(values.PERIOD_ORDINAL);
      ...
      const identityAssignments = context.assignments.filter((item) => item.schoolClassId === schoolClass.item!.id
        && item.subjectId === subject.item!.id && item.teacherUserId === teacher.id);
      if (identityAssignments.length === 0) { issues.push(this.issue('ASSIGNMENT_NOT_FOUND', sourceRow.number)); continue; }
      const covered = identityAssignments.filter((item) => formatCivilDate(item.validFrom) <= target.effectiveFrom
        && (!item.validUntil || formatCivilDate(item.validUntil) >= target.calendarEndDate));
      ...
      rows.push({
        sourceRowNumber: sourceRow.number,
        weekday,
        timeSlotDefinitionId: slot.id,
        schoolClassId: schoolClass.item.id,
        schoolClassCode: schoolClass.item.code,
        subjectId: subject.item.id,
        subjectCode: subject.item.code,
        teachingAssignmentId: assignment.id,
        teacherUserId: teacher.id,
        teacherDisplayName: teacher.profile.displayName,
        teacherStaffCode: teacher.profile.staffCode,
        normalizedSourceValues: values,
      });
      transient.push({
        id: `source-row-${sourceRow.number.toString().padStart(6, '0')}`,
    ...
    for (const validation of evaluateTimetableEntries({
      ...
      issues.push({
        ...
        relatedSourceRowNumbers: related.length > 1 ? related : undefined,
      });
    }
    ...
    const diff = blockingIssueCount === 0
      ? computePreviewDiff(rows.map(this.diffRow), baseline.entries.map(this.baselineDiffRow)) : null;
    return {
  ...
  private assertUniqueMappings(revision: RevisionContext): void {
    if (revision.columnMappings.length !== semanticFieldOrder.length
      ...
      throw new ConflictException({
        ...
        message: 'Profile revision must contain six unique mappings.',
      });
    }
  }
  ...
  private resolveCodeAlias<T extends { id: string }>(
    value: string,
    rows: T[],
    aliases: Array<{ sourceValueKey: string; schoolClassId: string | null; subjectId: string | null }>,
    code: keyof T,
  ) {
    const key = normalizeLookupKey(value);
    const ids = new Set<string>();
    const codeRow = rows.find((row) => normalizeLookupKey(String(row[code])) === key);
    if (codeRow) ids.add(codeRow.id);
    for (const alias of aliases) {
      if (alias.sourceValueKey !== key) continue;
      const id = alias.schoolClassId ?? alias.subjectId;
      if (id) ids.add(id);
    }
    return { item: ids.size === 1 ? rows.find((row) => row.id === [...ids][0]) : undefined, conflict: ids.size > 1 };
  }

  private issue(code: string, sourceRowNumber?: number, semanticField?: TimetableImportSemanticField, value?: string): TimetableImportPreviewIssue {
    return {
      ...
      ...(value ? { boundedSourceValue: value.slice(0, MAX_MAPPED_CELL_TEXT_LENGTH) } : {}),
    };
  }
    ...
    for (const group of grouped.values()) {
      if (group.length > 1) {
        for (const row of group) {
          issues.push({
            ...this.issue('DUPLICATE_CANONICAL_ROW', row.sourceRowNumber),
