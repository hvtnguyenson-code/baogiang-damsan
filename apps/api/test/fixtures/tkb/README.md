...
- **Zero Staff PII**: Real teacher display names from the authoritative workbook are not retained in this tracked fixture; teacher-view Column 1 values use deterministic synthetic identifiers `Giáo viên 01` through `Giáo viên 38`.
- **Zero Raw Teacher Codes**: Raw teacher codes from the authoritative workbook are intentionally omitted from tracked documentation and fixture data; all teacher-linked class-view markers use deterministic synthetic codes `GV01` through `GV38` (for example, `TO-GV02`, `SH-GV15`).
...
- **Privacy Rule**: Do not add real staff names, raw teacher codes, or other staff identity material from authoritative workbooks to this directory or any tracked repository path.
