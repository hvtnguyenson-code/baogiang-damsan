...
if (!common.includes('function Assert-ProductionRuntimeKindSupported') || !common.includes('SERVICE_FIRST_DEPLOY_UNSUPPORTED')) fail('shared first-deploy runtime-kind guard is missing');
if (!(preflight.indexOf('Assert-ProductionRuntimeKindSupported') < preflight.indexOf('Resolve-ExpectedCandidateRuntimeName'))) fail('preflight must reject unsupported first-deploy runtime kinds before candidate authority');
