...
if (!preflight.includes('Invoke-ReviewedPostgresEvidenceQuery')) fail('PASS 2 must invoke database evidence queries through Invoke-ReviewedPostgresEvidenceQuery');
if (/&\s*\$databaseVerifier\s+--tuples-only/i.test(preflight)) fail('PASS 2 must not bypass centralized Invoke-ReviewedPostgresEvidenceQuery authority');
