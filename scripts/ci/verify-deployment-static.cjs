...
const scheduledStart = restart.indexOf('Start-ScheduledTask');
const scheduledRuntimeCheck = restart.indexOf('Get-StartedProcess $startedAt', scheduledStart);
if (!(scheduledStart < scheduledRuntimeCheck && scheduledRuntimeCheck < restart.indexOf('Assert-ScheduledTaskHealthyState -Task $finalTask'))) fail('Scheduled Task post-activation verification must remain inside the activation lifecycle');
if (!restart.includes("Assert-ScheduledTaskHealthyState -Task $finalTask")) fail('final Scheduled Task healthy state must require Running');
...
if (!rollback.includes('Rollback health failure and Scheduled Task safe-stop cleanup failure') || !rollback.includes('Stop-ExactBaoGiangRuntime -Marker $marker')) fail('rollback health failure must own Scheduled Task safe-stop');
