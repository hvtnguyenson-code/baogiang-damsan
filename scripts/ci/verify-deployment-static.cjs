...
if (!(common.indexOf('function Assert-PathAncestorChainNonReparse') < common.indexOf('function Assert-SafeReadOnlyReportPath') && common.indexOf('Assert-PathAncestorChainNonReparse -Directory') > common.indexOf('function Assert-SafeReadOnlyReportPath'))) fail('report sink guard must delegate to the shared ancestor-chain authority');
