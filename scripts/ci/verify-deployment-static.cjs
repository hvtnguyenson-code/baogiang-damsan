...
for (const token of ['function Assert-OperatorEvidenceReportPath','function Assert-SafeDiscoveryReadPath','function Get-ReviewedExecutableSnapshot','OPERATOR_EVIDENCE_REPORT_PATH_CONFLICT','REVIEWED_EXECUTABLE_ROLE_LEAF_CONFLICT','function Get-ForeignDatabaseIsolationQuery','function Parse-PostgresStructuredEvidence','json_build_object','pg_auth_members','directMembershipCount','function Snapshot-PostgresProcessEnvironment','function Restore-PostgresProcessEnvironment','function Clear-PostgresProcessEnvironment','Get-ManagedPostgresEnvironmentNames','rolsuper','rolcreatedb','rolcreaterole','rolreplication','rolbypassrls','has_database_privilege']) if (!common.includes(token)) fail(`operator evidence authority missing: ${token}`);
