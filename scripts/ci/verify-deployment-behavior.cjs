...
assert.match(common, /function Assert-ProductionPositiveInteger/); assert.match(common, /\^\[1-9\]\[0-9\]\*\$/); assert.match(common, /AUTH_COOKIE_NAME.*\^\[A-Za-z0-9_-\]\+\$/); assert.match(common, /AUTH_COOKIE_PATH.*StartsWith\('\/'\)/); assert.match(common, /AUTH_COOKIE_SAME_SITE.*ToLowerInvariant/); assert.doesNotMatch(backup, /DatabaseUrlEnvironmentVariable/); assert.doesNotMatch(migration, /DatabaseUrlEnvironmentVariable/); assert.match(backup, /GetEnvironmentVariable\('DATABASE_URL','Process'\)/); assert.match(migration, /GetEnvironmentVariable\('DATABASE_URL','Process'\)/);
