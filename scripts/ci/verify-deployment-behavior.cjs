...
assert.match(discovery, /\{'AMBIGUOUS'\}/); assert.match(discovery, /authority='DISCOVERY'/); assert.match(discovery, /parserState='DISCOVERY'/); assert.match(discovery, /Assert-SafeDiscoveryReadPath/); assert.doesNotMatch(discovery, /function\s+Test-ContainedPath/);
...
for (const token of ['rolsuper','rolcreatedb','rolcreaterole','rolreplication','rolbypassrls','has_database_privilege','KnownForeignDatabaseRole','RequireForeignDatabaseIsolation']) assert.match(`${common}\n${preflight}`, new RegExp(token));
