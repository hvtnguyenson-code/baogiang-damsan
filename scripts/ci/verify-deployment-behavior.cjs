...
assert.match(common, /function Assert-ScheduledTaskHealthyState/); assert.match(restart, /AllowScheduledTaskActivation/); assert.match(restart, /Stop-ExactBaoGiangRuntime/); assert.ok(restart.indexOf('Enable-ScheduledTask') < restart.indexOf('Start-ScheduledTask')); assert.ok(restart.indexOf('Start-ScheduledTask') < restart.indexOf('Get-StartedProcess $startedAt', restart.indexOf('Start-ScheduledTask'))); assert.match(restart, /Assert-ScheduledTaskHealthyState -Task \$finalTask/);
...
assert.match(rollback, /Stop-ExactBaoGiangRuntime -Marker \$marker/); assert.match(rollback, /Rollback health failure and Scheduled Task safe-stop cleanup failure/);
