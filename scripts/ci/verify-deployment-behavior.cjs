...
const remoteSource = read('scripts/ci/build-windows-remote-command.cjs');
const evidenceSource = read('scripts/ci/resolve-startup-bundle-handshake-evidence.cjs');
...
assert.match(common, /function Get-DeploymentMarkerAuthorityContractVersion \{ return 1 \}/); assert.match(common, /PRODUCTION_ROOT_ANCESTOR_REPARSE_POINT/); assert.match(common, /PRODUCTION_ROOT_ANCESTOR_UNVERIFIABLE/);
assert.match(remoteSource, /Read-DeploymentIdentity/); assert.doesNotMatch(remoteSource, /Get-Content[^\n]*deployment-identity|ConvertFrom-Json|\$marker\.(?:systemId|canonicalRoot|domain|apiPort|service)/);
const trustedAuthoritySource = remoteSource.slice(remoteSource.indexOf('function trustedAuthorityScript'), remoteSource.indexOf('function buildHandshake'));
assert.match(trustedAuthoritySource, /\[IO\.FileStream\]::new\(\$common/); assert.match(trustedAuthoritySource, /\[IO\.FileShare\]::Read/); assert.match(trustedAuthoritySource, /\[Text\.UTF8Encoding\]::new\(\$false,\$true\)/); assert.match(trustedAuthoritySource, /\[ScriptBlock\]::Create\(\$commonText\)/); assert.match(trustedAuthoritySource, /\. \$trustedCommon/);
assert.doesNotMatch(trustedAuthoritySource, /\.\s+\$common(?:\W|$)|&\s*\$common(?:\W|$)|ReadAllBytes\(\$common\)|Get-Content[^\n]*\$common|powershell(?:\.exe)?[^\n]*-File[^\n]*\$common/i); assert.equal((trustedAuthoritySource.match(/\[IO\.FileStream\]::new\(\$common/g) || []).length, 1);
assert.ok(trustedAuthoritySource.indexOf('ComputeHash($commonBytes)') < trustedAuthoritySource.indexOf('[ScriptBlock]::Create($commonText)')); assert.ok(trustedAuthoritySource.indexOf('[ScriptBlock]::Create($commonText)') < trustedAuthoritySource.indexOf('. $trustedCommon')); assert.doesNotMatch(common, /\$PSScriptRoot/);
for (const field of ['wrapperPath','wrapperSha256','commonPath','commonSha256']) assert.match(trustedAuthoritySource, new RegExp(`identity\\.marker\\.startupBundle\\.${field}`));
assert.ok(remoteSource.indexOf('Read-DeploymentIdentity') < remoteSource.indexOf('New-Item -ItemType Directory'));
assert.match(remoteSource, /CLEANUP_REPARSE_POINT/); assert.ok(remoteSource.indexOf('CLEANUP_REPARSE_POINT') < remoteSource.indexOf('Remove-Item -LiteralPath $candidate'));
assert.match(evidenceSource, /cat-file/); assert.match(evidenceSource, /encoding = 'utf8'/); assert.match(evidenceSource, /encoding, maxBuffer/); assert.match(evidenceSource, /crypto\.createHash\('sha256'\)/); assert.match(evidenceSource, /rev-list.*--first-parent/);
assert.ok(workflow.indexOf('Revalidate authority and prepare unique transfer directory') < workflow.indexOf('Upload reviewed transfer bundle through SFTP'));
...
assert.match(discovery, /mode\s*=\s*'READ_ONLY_DISCOVERY'/); assert.match(discovery, /mutationsPerformed\s*=\s*\$false/); assert.match(discovery, /databaseAuthenticationAttempted\s*=\s*\$false/); assert.match(discovery, /conclusion\s*=\s*'REQUIRES_REVIEW'/);
assert.match(common, /function Assert-OperatorEvidenceReportPath/); assert.match(common, /function Get-ReviewedExecutableSnapshot/); assert.match(common, /function Assert-SafeDiscoveryReadPath/);
for (const tool of [discovery, preflight]) { assert.ok(tool.lastIndexOf('Assert-OperatorEvidenceReportPath') < tool.lastIndexOf('[IO.File]::WriteAllText')); }
assert.match(discovery, /'AMBIGUOUS'/); assert.match(discovery, /authority\s*=\s*'DISCOVERY'/); assert.match(discovery, /parserState\s*=\s*'DISCOVERY'/); assert.match(discovery, /Assert-SafeDiscoveryReadPath/); assert.doesNotMatch(discovery, /function\s+Test-ContainedPath/);
...
for (const token of ['rolsuper','rolcreatedb','rolcreaterole','rolreplication','rolbypassrls','has_database_privilege','KnownForeignDatabaseRole','RequireForeignDatabaseIsolation','pg_auth_members','directMembershipCount']) assert.match(`${common}\n${preflight}`, new RegExp(token));
...
assert.match(common, /function Normalize-NginxExactServerName/); assert.match(common, /TrimEnd\('\.'\)\.ToLowerInvariant\(\)/); assert.match(common, /function Get-NginxServerNameClassification/); assert.match(common, /kind='EXACT'/); assert.match(common, /kind='WILDCARD'/); assert.match(common, /kind='REGEX'/); assert.doesNotMatch(common, /\$names -ccontains \$Domain/);
assert.match(common, /function Assert-NginxPlanSchema/); assert.match(common, /function Assert-NginxRollbackSnapshotEvidence/); assert.match(common, /NGINX_ROLLBACK_SNAPSHOT_PROTECTED_LEAF/); assert.match(common, /NGINX_ROLLBACK_SNAPSHOT_HASH_MISMATCH/);
...
for (const nginxTool of [nginxPlan, nginxVerify]) { assert.match(nginxTool, /Assert-NginxPlanSchema/); assert.match(nginxTool, /Assert-NginxRollbackSnapshotEvidence/); }
assert.ok(common.indexOf('NGINX_ROLLBACK_SNAPSHOT_PROTECTED_LEAF') < common.indexOf('Get-FileSha256FromBytes $snapshot')); assert.ok(nginxVerify.indexOf('Assert-NginxRollbackSnapshotEvidence') < nginxVerify.indexOf('Invoke-ReviewedNginxSyntaxTest')); assert.match(nginxVerify, /RequireExact/);
