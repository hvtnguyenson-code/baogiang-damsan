...
assert.match(common, /function Get-ManagedProductionEnvironmentNames/); assert.match(common, /function Read-ValidatedProductionEnvironment/); assert.match(common, /function Restore-ServerEnvironment/); assert.match(common, /StringComparer\]::OrdinalIgnoreCase/); assert.match(common, /foreach \(\$name in \$required\)/); assert.match(common, /foreach \(\$name in Get-ManagedProductionEnvironmentNames\)/); assert.ok(common.indexOf('Read-ValidatedProductionEnvironment') < common.indexOf('SetEnvironmentVariable'));
