...
assert.match(common, /commandLineRedacted/); assert.match(inventory, /Get-NormalizedProcessIdentity/); assert.doesNotMatch(inventory, /Get-ListenerSnapshot 22/);
...
assert.match(workflow, /Retrieve redacted deploy report/); assert.match(workflow, /upload-artifact@v4/); assert.match(workflow, /if: always\(\)/);
