...
$missingMigrations = Get-DatabaseEvidenceClassification -ActualDatabase 'baogiang' -ExpectedDatabase 'baogiang' -ActualRole 'baogiang_app' -ExpectedRole 'baogiang_app' -ActualExtensions @('btree_gist') -RequiredExtensions @('btree_gist') -MigrationTablePresent $false -RoleSafetyVerified $true
...
$cleanMigrations = Get-DatabaseEvidenceClassification -ActualDatabase 'baogiang' -ExpectedDatabase 'baogiang' -ActualRole 'baogiang_app' -ExpectedRole 'baogiang_app' -ActualExtensions @('btree_gist') -RequiredExtensions @('btree_gist') -MigrationTablePresent $true -MigrationSummaryVerified $true -RoleSafetyVerified $true
if ($cleanMigrations.state -ne 'EXISTS AND VERIFIED') { throw 'Clean migration classification failed.' }
$unfinished = Get-DatabaseEvidenceClassification -ActualDatabase 'baogiang' -ExpectedDatabase 'baogiang' -ActualRole 'baogiang_app' -ExpectedRole 'baogiang_app' -ActualExtensions @('btree_gist') -RequiredExtensions @('btree_gist') -MigrationTablePresent $true -MigrationSummaryVerified $true -UnfinishedMigrations 1 -RoleSafetyVerified $true
...
$summaryUnavailable = Get-DatabaseEvidenceClassification -ActualDatabase 'baogiang' -ExpectedDatabase 'baogiang' -ActualRole 'baogiang_app' -ExpectedRole 'baogiang_app' -ActualExtensions @('btree_gist') -RequiredExtensions @('btree_gist') -MigrationTablePresent $true -MigrationSummaryVerified $false -RoleSafetyVerified $true
...
try {
  ...
  $p9Root = Join-Path $temp 'ope-p9-root'
  foreach ($sub in @('releases','staging','incoming','shared','logs','backups')) { New-Item -ItemType Directory (Join-Path $p9Root $sub) -Force | Out-Null }
  $p9Sha = 'b' * 40
  $p9BundleDir = Join-Path $p9Root "shared\startup-bundles\$p9Sha"
  New-Item -ItemType Directory -Path $p9BundleDir -Force | Out-Null
  $p9Wrapper = Join-Path $p9BundleDir 'start-baogiang-api.ps1'
  $p9Common = Join-Path $p9BundleDir 'deployment-common.ps1'
  [IO.File]::WriteAllText($p9Wrapper, (Get-Content -LiteralPath (Join-Path $repo 'scripts\deploy\windows\start-baogiang-api.ps1') -Raw), [Text.UTF8Encoding]::new($false))
  [IO.File]::WriteAllText($p9Common, (Get-Content -LiteralPath (Join-Path $repo 'scripts\deploy\windows\deployment-common.ps1') -Raw), [Text.UTF8Encoding]::new($false))
  $p9Env = Join-Path $p9Root 'shared\production.env'; [IO.File]::WriteAllLines($p9Env, $validEnvLines, [Text.UTF8Encoding]::new($false))
  $p9Node = Join-Path $reviewedTools 'node.exe'
  $p9Nginx = Join-Path $reviewedTools 'nginx.exe'
  $p9NginxConfig = Join-Path $nginxConf 'nginx.conf'
  $p9EntryPoint = Join-Path $p9Root 'current\apps\api\dist\main.js'
  $p9Task = [ordered]@{ kind = 'scheduled-task'; name = 'BaoGiangTask'; taskPath = '\BaoGiang\'; account = 'fixture'; execute = $powershellExe; arguments = "-File `"$p9Wrapper`""; workingDirectory = (Join-Path $p9Root 'shared') }
  $p9Marker = [ordered]@{
    ...
    nodeExe = $p9Node; envFile = $p9Env; startupWrapper = $p9Wrapper; entryPoint = $p9EntryPoint; nginxExe = $p9Nginx; nginxConfig = $p9NginxConfig
    foreignIsolation = [ordered]@{ reviewedNginxPrefix = $nginxRoot; reviewedNginxConfig = $p9NginxConfig; foreignRoots = @((Join-Path $temp 'foreign')); bootstrapReportReference = 'fixture' }
  ...
  $p9Trigger = [pscustomobject]@{ CimClass = [pscustomobject]@{ CimClassName = 'MSFT_TaskBootTrigger' }; Enabled = $true }
  function global:Get-ScheduledTask { @([pscustomobject]@{ TaskName = 'BaoGiangTask'; TaskPath = '\BaoGiang\'; State = 'Disabled'; Principal = [pscustomobject]@{ UserId = 'fixture' }; Actions = @([pscustomobject]@{ Execute = $powershellExe; Arguments = "-File `"$p9Wrapper`""; WorkingDirectory = (Join-Path $p9Root 'shared') }); Triggers = @($p9Trigger) }) }
  function global:Get-NetTCPConnection { @() }
  $p9ControlReport = Join-Path $opeEvidence 'p9-control.json'
  & $preflightScript -ReportPath $p9ControlReport -CandidateRoot $p9Root -RequireVerifiedIdentity -ExpectedTaskName 'BaoGiangTask' -ServiceKind 'scheduled-task' -EnvFile $p9Env -StartupWrapper $p9Wrapper -ExpectedEntryPoint $p9EntryPoint -NodeExe $p9Node -NginxExe $p9Nginx -NginxPrefix $nginxRoot -NginxConfig $p9NginxConfig | Out-Null
  $p9ControlJson = Get-Content -LiteralPath $p9ControlReport -Raw | ConvertFrom-Json
  if ($p9ControlJson.identity.status -ne 'EXISTS AND VERIFIED') { throw 'OPE-P9 control preflight identity verification failed' }
  $p9AltDir = Join-Path $temp 'p9-alt-node'
  New-Item -ItemType Directory -Path $p9AltDir -Force | Out-Null
  $p9MismatchNode = Join-Path $p9AltDir 'node.exe'
  Copy-Item $powershellExe $p9MismatchNode
  $p9MismatchReport = Join-Path $opeEvidence 'p9-mismatch.json'
  $p9Err = $null
  try {
    & $preflightScript -ReportPath $p9MismatchReport -CandidateRoot $p9Root -RequireVerifiedIdentity -ExpectedTaskName 'BaoGiangTask' -ServiceKind 'scheduled-task' -EnvFile $p9Env -StartupWrapper $p9Wrapper -ExpectedEntryPoint $p9EntryPoint -NodeExe $p9MismatchNode -NginxExe $p9Nginx -NginxPrefix $nginxRoot -NginxConfig $p9NginxConfig 2>&1 | Out-Null
  } catch {
    $p9Err = $_.Exception.Message
  }
  if ($p9Err -notmatch 'Deployment identity marker Node executable mismatch') { throw "OPE-P9 expected Node executable mismatch rejection, got: $p9Err" }
  Set-OpePass 'OPE-P9'
  ...
  # OPE-P15: production-authoritative foreign database pipeline
  $fQuery = Get-ForeignDatabaseIsolationQuery -DatabaseName 'damsan_foreign'
  if ($fQuery -notmatch 'damsan_foreign') { throw 'OPE-P15 query generation failed' }
  # CONNECT=true -> CONFLICT
  $fConnectTrueLines = @('{"record":"foreignDatabase","database":"damsan_foreign","present":true,"connect":true}')
  $fParsedTrue = Parse-PostgresStructuredEvidence -Lines $fConnectTrueLines
  $fEvidenceTrue = ConvertTo-ReviewedForeignDatabaseEvidence -RequestedDatabase 'damsan_foreign' -ParsedEvidence $fParsedTrue
  if ($fEvidenceTrue.state -ne 'CONFLICT' -or $fEvidenceTrue.existence -ne 'EXISTS') { throw 'OPE-P15 foreign CONNECT=true was not CONFLICT' }
  $fConnectTrueClass = Get-DatabaseEvidenceClassification -ActualDatabase 'baogiang' -ExpectedDatabase 'baogiang' -ActualRole 'baogiang_app' -ExpectedRole 'baogiang_app' -ActualExtensions @('btree_gist') -RequiredExtensions @('btree_gist') -MigrationTablePresent:$false -RoleSafetyVerified:$true -RequireForeignIsolation:$true -ForeignIsolation @($fEvidenceTrue)
  if ($fConnectTrueClass.state -ne 'CONFLICT') { throw 'OPE-P15 classification CONNECT=true was not CONFLICT' }
  # CONNECT=false -> PASS/PARTIAL
  $fConnectFalseLines = @('{"record":"foreignDatabase","database":"damsan_foreign","present":true,"connect":false}')
  $fParsedFalse = Parse-PostgresStructuredEvidence -Lines $fConnectFalseLines
  $fEvidenceFalse = ConvertTo-ReviewedForeignDatabaseEvidence -RequestedDatabase 'damsan_foreign' -ParsedEvidence $fParsedFalse
  if ($fEvidenceFalse.state -ne 'PASS' -or $fEvidenceFalse.existence -ne 'EXISTS') { throw 'OPE-P15 foreign CONNECT=false was not PASS' }
  $fConnectFalseClass = Get-DatabaseEvidenceClassification -ActualDatabase 'baogiang' -ExpectedDatabase 'baogiang' -ActualRole 'baogiang_app' -ExpectedRole 'baogiang_app' -ActualExtensions @('btree_gist') -RequiredExtensions @('btree_gist') -MigrationTablePresent:$false -RoleSafetyVerified:$true -RequireForeignIsolation:$true -ForeignIsolation @($fEvidenceFalse)
  if ($fConnectFalseClass.state -ne 'PARTIAL') { throw 'OPE-P15 classification CONNECT=false was not PARTIAL (migration unapplied)' }
  # MISSING -> NOT_VERIFIED
  $fMissingLines = @('{"record":"foreignDatabase","database":"damsan_foreign","present":false,"connect":false}')
  $fParsedMissing = Parse-PostgresStructuredEvidence -Lines $fMissingLines
  $fEvidenceMissing = ConvertTo-ReviewedForeignDatabaseEvidence -RequestedDatabase 'damsan_foreign' -ParsedEvidence $fParsedMissing
  if ($fEvidenceMissing.state -ne 'NOT_VERIFIED' -or $fEvidenceMissing.existence -ne 'MISSING') { throw 'OPE-P15 missing foreign database was not NOT_VERIFIED' }
  $fMissingClass = Get-DatabaseEvidenceClassification -ActualDatabase 'baogiang' -ExpectedDatabase 'baogiang' -ActualRole 'baogiang_app' -ExpectedRole 'baogiang_app' -ActualExtensions @('btree_gist') -RequiredExtensions @('btree_gist') -MigrationTablePresent:$false -RoleSafetyVerified:$true -RequireForeignIsolation:$true -ForeignIsolation @($fEvidenceMissing)
  if ($fMissingClass.foreignIsolationState -ne 'NOT_VERIFIED') { throw 'OPE-P15 missing foreign isolation classification was not NOT_VERIFIED' }
  Set-OpePass 'OPE-P15'
  ...
  # OPE-P29: Mixed Nginx processes (1 proven -p + 1 unproven no -p) -> AMBIGUOUS / NOT_VERIFIED, no active config read
  $p29PrefixA = Join-Path $temp 'p29-nginx-prefix-a'
  $p29ConfA = Join-Path $p29PrefixA 'conf'
  New-Item -ItemType Directory -Path $p29ConfA -Force | Out-Null
  [IO.File]::WriteAllText((Join-Path $p29ConfA 'nginx.conf'), "events {}`nhttp { server { listen 8089; server_name p29.test; } }`n", [Text.UTF8Encoding]::new($false))
  $global:p29Processes = @(
    [pscustomobject]@{ ProcessId = 3291; ParentProcessId = 1; Name = 'nginx.exe'; ExecutablePath = $p20Exe; CommandLine = "nginx.exe -p `"$p29PrefixA`"" },
    [pscustomobject]@{ ProcessId = 3292; ParentProcessId = 1; Name = 'nginx.exe'; ExecutablePath = $p20Exe; CommandLine = 'nginx.exe' }
  )
  function global:Get-CimInstance([string]$ClassName, [string]$Filter) {
    if ($ClassName -eq 'Win32_Process') { return $global:p29Processes }
    if ($ClassName -eq 'Win32_Service') { return @() }
    [pscustomobject]@{ Caption = 'Fixture Windows' }
  }
  $p29Report = Join-Path $opeEvidence 'p29.json'
  & $neighborDiscoveryPath -ReportPath $p29Report -CandidateBaoGiangRoot $opeCandidate -NginxRoot $nginxRoot | Out-Null
  $p29Json = Get-Content -LiteralPath $p29Report -Raw -Encoding UTF8 | ConvertFrom-Json
  if ($p29Json.nginx.state -notin @('AMBIGUOUS','NOT_VERIFIED') -or @($p29Json.nginx.configFiles).Count -ne 0 -or @($p29Json.nginx.serverBlocks).Count -ne 0) {
    throw 'OPE-P29 mixed Nginx bindings allowed unique active config authority'
  }
  Set-OpePass 'OPE-P29'
  Remove-Item Function:\Get-CimInstance -Force

  # OPE-P30A: Valid marker + mismatched NginxPrefix -> exact rejection
  $p30aReport = Join-Path $opeEvidence 'p30a.json'
  $p30aWrongPrefix = Join-Path $temp 'wrong-nginx-prefix'
  New-Item -ItemType Directory -Path $p30aWrongPrefix -Force | Out-Null
  $p30aErr = $null
  try {
    & $preflightScript -ReportPath $p30aReport -CandidateRoot $p9Root -RequireVerifiedIdentity -ExpectedTaskName 'BaoGiangTask' -ServiceKind 'scheduled-task' -EnvFile $p9Env -StartupWrapper $p9Wrapper -ExpectedEntryPoint $p9EntryPoint -NodeExe $p9Node -NginxExe $p9Nginx -NginxPrefix $p30aWrongPrefix -NginxConfig $p9NginxConfig 2>&1 | Out-Null
  } catch {
    $p30aErr = $_.Exception.Message
  }
  if ($p30aErr -notmatch 'Deployment identity marker Nginx prefix mismatch') { throw "OPE-P30A expected Nginx prefix mismatch rejection, got: $p30aErr" }

  # OPE-P30B: Reviewed NginxConfig + missing NginxPrefix -> no exe-directory fallback; rejection
  $p30bReport = Join-Path $opeEvidence 'p30b.json'
  $p30bErr = $null
  try {
    & $preflightScript -ReportPath $p30bReport -CandidateRoot $opeCandidate -NginxExe (Join-Path $reviewedTools 'nginx.exe') -NginxConfig (Join-Path $nginxConf 'nginx.conf') 2>&1 | Out-Null
  } catch {
    $p30bErr = $_.Exception.Message
  }
  if ($p30bErr -notmatch 'NginxConfig requires a reviewed absolute NginxPrefix') { throw "OPE-P30B expected NginxConfig requires reviewed NginxPrefix rejection, got: $p30bErr" }
  Set-OpePass 'OPE-P30'

  # OPE-P31: Full PostgreSQL 17 libpq environment isolation and exception safety
  $hostileVars = @{
    PGSSLNEGOTIATION = 'direct'
    PGSSLCERTMODE = 'allow'
    PGGSSENCMODE = 'require'
    PGKRBSRVNAME = 'hostile_service'
    PGLOADBALANCEHOSTS = 'random'
    PGSSLCOMPRESSION = '1'
    PGREQUIREPEER = 'hostile_user'
    PGSSLMINPROTOCOLVERSION = 'TLSv1.2'
    PGSSLMAXPROTOCOLVERSION = 'TLSv1.3'
    PGGSSLIB = 'gssapi'
    PGGSSDELEGATION = '1'
    PGSYSCONFDIR = 'C:\hostile\conf'
  }
  foreach ($k in $hostileVars.Keys) { [Environment]::SetEnvironmentVariable($k, $hostileVars[$k], 'Process') }
  try {
    $p31Snapshot = Snapshot-PostgresProcessEnvironment
    $p31Parts = Set-PostgresProcessEnvironment -DatabaseUrl 'postgresql://app:secret@127.0.0.1:5433/baogiang' -ExpectedPort 5433
    foreach ($k in $hostileVars.Keys) {
      if ($null -ne [Environment]::GetEnvironmentVariable($k, 'Process')) {
        throw "OPE-P31 Set-PostgresProcessEnvironment did not clear hostile variable $k"
      }
    }
  } finally {
    Restore-PostgresProcessEnvironment -Snapshot $p31Snapshot
  }
  foreach ($k in $hostileVars.Keys) {
    if ([Environment]::GetEnvironmentVariable($k, 'Process') -ne $hostileVars[$k]) {
      throw "OPE-P31 Restore-PostgresProcessEnvironment did not restore variable $k"
    }
    [Environment]::SetEnvironmentVariable($k, $null, 'Process')
  }
  # Test restoration on exception inside database lifecycle
  foreach ($k in $hostileVars.Keys) { [Environment]::SetEnvironmentVariable($k, $hostileVars[$k], 'Process') }
  $p31SnapEx = Snapshot-PostgresProcessEnvironment
  try {
    try {
      ...
      Restore-PostgresProcessEnvironment -Snapshot $p31SnapEx
    }
  } catch {
    if ($_.Exception.Message -ne 'SIMULATED_DB_ERROR') { throw }
  }
  foreach ($k in $hostileVars.Keys) {
    if ([Environment]::GetEnvironmentVariable($k, 'Process') -ne $hostileVars[$k]) {
      throw "OPE-P31 Exception restore failed for $k"
    }
    [Environment]::SetEnvironmentVariable($k, $null, 'Process')
  }
  Set-OpePass 'OPE-P31'

  # OPE-P32: ConvertTo-ReviewedForeignDatabaseEvidence binds exact requested database name
  $p32MismatchedLines = @('{"record":"foreignDatabase","database":"other_database","present":true,"connect":false}')
  $p32Parsed = Parse-PostgresStructuredEvidence -Lines $p32MismatchedLines
  $p32Err = $null
  try {
    ConvertTo-ReviewedForeignDatabaseEvidence -RequestedDatabase 'damsan' -ParsedEvidence $p32Parsed | Out-Null
  } catch {
    $p32Err = $_.Exception.Message
  }
  if ($p32Err -ne 'DATABASE_FOREIGN_EVIDENCE_BINDING_CONFLICT') {
    throw "OPE-P32 expected DATABASE_FOREIGN_EVIDENCE_BINDING_CONFLICT, got: $p32Err"
  }
  Set-OpePass 'OPE-P32'

  # OPE-P33: PASS 2 protects ReviewedPostgresDataDirectory from ReportPath write
  $p33PgData = Join-Path $temp 'p33-postgres-data'
  New-Item -ItemType Directory -Path $p33PgData -Force | Out-Null
  $p33Sentinel = Join-Path $p33PgData 'p33-report.json'
  $p33Err = $null
  try {
    & $preflightScript -ReportPath $p33Sentinel -CandidateRoot $opeCandidate -ReviewedPostgresDataDirectory $p33PgData 2>&1 | Out-Null
  } catch {
    $p33Err = $_.Exception.Message
  }
  ...
  $payload = @{ root = 'C:\baogiang'; serviceKind = 'scheduled-task'; serviceName = "O'Brien"; envFile = 'C:\env'; startupWrapper = ('C:\baogiang\shared\startup-bundles\' + ('a' * 40) + '\start-baogiang-api.ps1'); expectedEntryPoint = 'C:\baogiang\current\api.js'; nodeExe = 'C:\node.exe'; nginxExe = 'C:\nginx.exe'; nginxConfig = 'C:\nginx.conf'; reviewedCommitSha = ('a' * 40); wrapperPath = ('C:\baogiang\shared\startup-bundles\' + ('a' * 40) + '\start-baogiang-api.ps1'); commonPath = ('C:\baogiang\shared\startup-bundles\' + ('a' * 40) + '\deployment-common.ps1'); wrapperSha256 = ('b' * 64); commonSha256 = ('c' * 64); markerAuthorityContractVersion = 1 } | ConvertTo-Json -Compress
  ...
  if ($cleanupDecoded -notmatch 'CLEANUP_NOT_DIRECT_CHILD' -or $cleanupDecoded -notmatch 'CLEANUP_REPARSE_POINT') { throw 'Cleanup containment contract failed.' }
  ...
  function Assert-NgxRejectCategory([scriptblock]$Action,[string]$Id,[string]$Category){$message='';try{& $Action|Out-Null}catch{$message=$_.Exception.Message};if($message -notmatch [regex]::Escape($Category)){throw "$Id expected $Category but received: $message"};Set-NgxPass $Id}
  function Invoke-NgxPlanFixture([string]$Path,[string]$Managed=$nginxManaged,[string]$Main=$nginxMain,[string]$Snapshot='',[string]$Certificate=$nginxCertificate,[string]$PrivateKey=$nginxPrivateKey) {
    & $nginxPlanScript -RepositoryRoot $repo -Root $nginxProductionRoot -NginxExe $nginxExe -NginxPrefix $nginxPrefix -NginxConfig $Main -ManagedConfig $Managed -TlsCertificate $Certificate -TlsPrivateKey $PrivateKey -ClientMaxBodySize '20m' -RollbackSnapshot $Snapshot -ReportPath $Path | Out-Null
  ...
  [IO.File]::WriteAllBytes($nginxManaged,$desiredBytes);Remove-Item $snapshot;Assert-NgxRejectCategory {Invoke-NgxVerifyFixture 'Desired' $existingPlanPath (Join-Path $nginxEvidence 'p24.json')} 'NGX-P24' 'NGINX_ROLLBACK_SNAPSHOT_MISSING';[IO.File]::WriteAllBytes($snapshot,$existingOriginal)
  [IO.File]::WriteAllText($snapshot,'tampered rollback bytes',[Text.UTF8Encoding]::new($false));Assert-NgxRejectCategory {Invoke-NgxVerifyFixture 'Desired' $existingPlanPath (Join-Path $nginxEvidence 'p25.json')} 'NGX-P25' 'NGINX_ROLLBACK_SNAPSHOT_HASH_MISMATCH';[IO.File]::WriteAllBytes($snapshot,$existingOriginal)
  [IO.File]::WriteAllBytes($nginxManaged,$existingOriginal);$snapshotContainer=Join-Path $nginxEvidence 'snapshot-container';$snapshotTargetDirectory=Join-Path $nginxEvidence 'snapshot-target-directory';New-Item -ItemType Directory -Path $snapshotContainer,$snapshotTargetDirectory|Out-Null;$reparseSnapshot=Join-Path $snapshotContainer 'rollback.snapshot';[IO.File]::WriteAllBytes($reparseSnapshot,$existingOriginal);[IO.File]::WriteAllBytes((Join-Path $snapshotTargetDirectory 'rollback.snapshot'),$existingOriginal);$reparsePlanPath=Join-Path $nginxEvidence 'p26-plan.json';if((Invoke-NgxPlanFixture $reparsePlanPath $nginxManaged $nginxMain $reparseSnapshot).state -cne 'READY_FOR_MANUAL_APPLY'){throw 'NGX-P26 setup plan not READY'};[IO.File]::WriteAllBytes($nginxManaged,$desiredBytes);Remove-Item $reparseSnapshot;[IO.Directory]::Delete($snapshotContainer);New-Item -ItemType Junction -Path $snapshotContainer -Target $snapshotTargetDirectory|Out-Null;Assert-NgxRejectCategory {Invoke-NgxVerifyFixture 'Desired' $reparsePlanPath (Join-Path $nginxEvidence 'p26.json')} 'NGX-P26' 'NGINX_ROLLBACK_SNAPSHOT_REPARSE_POINT';[IO.Directory]::Delete($snapshotContainer)
  [IO.File]::WriteAllBytes($nginxManaged,$existingOriginal);$externalPrivateKey=Join-Path $nginxFixture 'external-private-key.pem';[IO.File]::WriteAllBytes($externalPrivateKey,$existingOriginal);$privateKeyLock=[IO.File]::Open($externalPrivateKey,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::None)
  try { Assert-NgxRejectCategory {Invoke-NgxPlanFixture (Join-Path $nginxEvidence 'p27.json') $nginxManaged $nginxMain $externalPrivateKey $nginxCertificate $externalPrivateKey|Out-Null} 'NGX-P27' 'NGINX_ROLLBACK_SNAPSHOT_PROTECTED_LEAF' } finally { $privateKeyLock.Dispose() }
  [IO.File]::WriteAllBytes($nginxManaged,$desiredBytes);$tamperedReadyPlan=Join-Path $nginxEvidence 'p28-plan.json';$tamperedPlan=Get-Content $existingPlanPath -Raw -Encoding UTF8|ConvertFrom-Json;$tamperedPlan.rollbackSnapshot.state='NOT_REQUIRED';$tamperedPlan.rollbackSnapshot.path=$null;$tamperedPlan.rollbackSnapshot.sha256=$null;[IO.File]::WriteAllText($tamperedReadyPlan,($tamperedPlan|ConvertTo-Json -Depth 12),[Text.UTF8Encoding]::new($false));Assert-NgxRejectCategory {Invoke-NgxVerifyFixture 'Desired' $tamperedReadyPlan (Join-Path $nginxEvidence 'p28.json')} 'NGX-P28' 'NGINX_PLAN_SCHEMA_INVALID'
  $hostileMissingPlan=Join-Path $nginxEvidence 'p29-plan.json';$hostileMissing=Get-Content $missingPlanPath -Raw -Encoding UTF8|ConvertFrom-Json;$hostileMissing.rollbackSnapshot.state='EXACT';$hostileMissing.rollbackSnapshot.path=$snapshot;$hostileMissing.rollbackSnapshot.sha256=('a'*64);[IO.File]::WriteAllText($hostileMissingPlan,($hostileMissing|ConvertTo-Json -Depth 12),[Text.UTF8Encoding]::new($false));Assert-NgxRejectCategory {Invoke-NgxVerifyFixture 'Desired' $hostileMissingPlan (Join-Path $nginxEvidence 'p29.json')} 'NGX-P29' 'NGINX_PLAN_SCHEMA_INVALID'
  $sameHashSubstitute=Join-Path $nginxEvidence 'same-hash-substitute.snapshot';[IO.File]::WriteAllBytes($sameHashSubstitute,$existingOriginal);Remove-Item $snapshot;Assert-NgxRejectCategory {Invoke-NgxVerifyFixture 'Desired' $existingPlanPath (Join-Path $nginxEvidence 'p30.json')} 'NGX-P30' 'NGINX_ROLLBACK_SNAPSHOT_MISSING';[IO.File]::WriteAllBytes($snapshot,$existingOriginal)
  $currentManagedSnapshot=Join-Path $nginxEvidence 'current-managed.snapshot';[IO.File]::WriteAllBytes($currentManagedSnapshot,$desiredBytes);$normalizedDuplicate=Join-Path $nginxConfD 'normalized-duplicate.conf'
  foreach($hostnameCase in @(
    @{id='NGX-P31';name='BAOGIANG.DTNT-DAMSAN.EDU.VN'},
    @{id='NGX-P32';name='BaoGiang.Dtnt-Damsan.Edu.Vn'},
    @{id='NGX-P33';name='baogiang.dtnt-damsan.edu.vn.'},
    @{id='NGX-P34';name='BAOGIANG.DTNT-DAMSAN.EDU.VN.'}
  )){[IO.File]::WriteAllText($normalizedDuplicate,"server { listen 443 ssl; server_name $($hostnameCase.name); }`n",[Text.UTF8Encoding]::new($false));$normalizedPlan=Invoke-NgxPlanFixture (Join-Path $nginxEvidence "$($hostnameCase.id)-plan.json") $nginxManaged $nginxMain $currentManagedSnapshot;if($normalizedPlan.state -cne 'CONFLICT' -or $normalizedPlan.reason -cne 'NGINX_DOMAIN_443_COLLISION'){throw "$($hostnameCase.id) normalized exact hostname was not a collision"};Set-NgxPass $hostnameCase.id};Remove-Item $normalizedDuplicate
  [IO.File]::WriteAllText($nginxNeighbor,"server { listen 80; server_name BAOGIANG.DTNT-DAMSAN.EDU.VN.; }`n",[Text.UTF8Encoding]::new($false));$port80Plan=Invoke-NgxPlanFixture (Join-Path $nginxEvidence 'p35-plan.json') $nginxManaged $nginxMain $currentManagedSnapshot;if($port80Plan.state -cne 'READY_FOR_MANUAL_APPLY' -or $port80Plan.reason -ne $null){throw 'NGX-P35 normalized HTTP-only hostname falsely collided'};Set-NgxPass 'NGX-P35';[IO.File]::WriteAllBytes($nginxNeighbor,$neighborOriginal)
  [IO.File]::WriteAllText($normalizedDuplicate,"server { listen 443 ssl; server_name baogiang2.dtnt-damsan.edu.vn; }`nserver { listen 443 ssl; server_name baogiang.dtnt-damsan.edu.vnx; }`n",[Text.UTF8Encoding]::new($false));$unrelatedPlan=Invoke-NgxPlanFixture (Join-Path $nginxEvidence 'p36-plan.json') $nginxManaged $nginxMain $currentManagedSnapshot;if($unrelatedPlan.state -cne 'READY_FOR_MANUAL_APPLY' -or $unrelatedPlan.reason -ne $null){throw 'NGX-P36 unrelated hostname falsely collided'};Set-NgxPass 'NGX-P36';Remove-Item $normalizedDuplicate
  foreach($classificationCase in @(@{token='baogiang.dtnt-damsan.edu.vn.';kind='EXACT'},@{token='*.dtnt-damsan.edu.vn';kind='WILDCARD'},@{token='.dtnt-damsan.edu.vn';kind='WILDCARD'},@{token='~^baogiang.dtnt-damsan.edu.vn$';kind='REGEX'})){$actualClassification=(Get-NginxServerNameClassification $classificationCase.token).kind;if($actualClassification -cne $classificationCase.kind){throw "Nginx server-name classification fixture failed: $($classificationCase.token) expected $($classificationCase.kind), received $actualClassification"}}
  [IO.File]::WriteAllText($normalizedDuplicate,"server { listen 443 ssl; server_name BAOGIANG.DTNT-DAMSAN.EDU.VN.; }`n",[Text.UTF8Encoding]::new($false));Remove-Item $nginxTrace -ErrorAction SilentlyContinue;Invoke-ReviewedNginxSyntaxTest $nginxExe $nginxPrefix $nginxMain|Out-Null;$warningIndependentPlan=Invoke-NgxPlanFixture (Join-Path $nginxEvidence 'p37-plan.json') $nginxManaged $nginxMain $currentManagedSnapshot;if(-not (Test-Path $nginxTrace) -or $warningIndependentPlan.state -cne 'CONFLICT' -or $warningIndependentPlan.reason -cne 'NGINX_DOMAIN_443_COLLISION'){throw 'NGX-P37 syntax success replaced custom collision authority'};Set-NgxPass 'NGX-P37';Remove-Item $normalizedDuplicate
  if($nginxResults.Count -ne 37 -or @($nginxResults.Values|Where-Object{$_ -ne 'PASS'}).Count -ne 0){throw 'NGX fixture matrix incomplete'}

  # P2 shared marker / pre-transfer handshake authority fixtures.
  $xferSha = 'd' * 40
  $xferRoot = Join-Path $repo ('.xfer-fixture-' + [Guid]::NewGuid().ToString('N'))
  foreach($name in @('releases','staging','incoming','shared','logs','backups')){New-Item -ItemType Directory -Path (Join-Path $xferRoot $name) -Force|Out-Null}
  $xferVersion = Join-Path $xferRoot "shared\startup-bundles\$xferSha"
  New-Item -ItemType Directory -Path $xferVersion -Force|Out-Null
  $xferWrapper = Join-Path $xferVersion 'start-baogiang-api.ps1';$xferCommon=Join-Path $xferVersion 'deployment-common.ps1'
  Copy-Item -LiteralPath (Join-Path $repo 'scripts\deploy\windows\start-baogiang-api.ps1') -Destination $xferWrapper
  Copy-Item -LiteralPath (Join-Path $repo 'scripts\deploy\windows\deployment-common.ps1') -Destination $xferCommon
  $xferNode=Join-Path $temp 'xfer-node.exe';$xferEnv=Join-Path $temp 'xfer.env';$xferNginx=Join-Path $temp 'xfer-nginx.exe';$xferConfig=Join-Path $temp 'xfer-nginx.conf';$xferTaskExe=Join-Path $temp 'xfer-task.exe'
  foreach($leaf in @($xferNode,$xferEnv,$xferNginx,$xferConfig,$xferTaskExe)){[IO.File]::WriteAllText($leaf,'fixture',[Text.UTF8Encoding]::new($false))}
  $xferMarkerPath=Join-Path $xferRoot 'shared\deployment-identity.json'
  $xferEntry=Join-Path $xferRoot 'current\apps\api\dist\apps\api\src\main.js'
  $xferMarker=[ordered]@{schemaVersion=[long]1;systemId='baogiang-damsan';canonicalRoot=(Get-CanonicalPath $xferRoot);domain='https://baogiang.dtnt-damsan.edu.vn';apiPort=[long]3100;nodeExe=$xferNode;envFile=$xferEnv;startupWrapper=$xferWrapper;entryPoint=$xferEntry;nginxExe=$xferNginx;nginxConfig=$xferConfig;foreignIsolation=[ordered]@{reviewedNginxPrefix=(Join-Path $temp 'xfer-nginx-prefix');reviewedNginxConfig=$xferConfig;foreignRoots=@((Join-Path $temp 'xfer-foreign'));bootstrapReportReference='fixture'};startupBundle=[ordered]@{wrapperPath=$xferWrapper;wrapperSha256=(Get-FileSha256FromBytes $xferWrapper);commonPath=$xferCommon;commonSha256=(Get-FileSha256FromBytes $xferCommon)};service=[ordered]@{kind='scheduled-task';name='BaoGiangBackend';taskPath='\BaoGiang\';account='fixture';execute=$xferTaskExe;arguments='-File start-baogiang-api.ps1';workingDirectory=(Join-Path $xferRoot 'shared')}}
  function Write-XferMarker($Marker=$xferMarker){[IO.File]::WriteAllText($xferMarkerPath,($Marker|ConvertTo-Json -Depth 10),[Text.UTF8Encoding]::new($false))}
  function Get-XferContract([string]$TransferName=''){$contract=[ordered]@{root=(Get-CanonicalPath $xferRoot);serviceKind='scheduled-task';serviceName='BaoGiangBackend';envFile=$xferEnv;startupWrapper=$xferWrapper;expectedEntryPoint=$xferEntry;nodeExe=$xferNode;nginxExe=$xferNginx;nginxConfig=$xferConfig;reviewedCommitSha=$xferSha;wrapperPath=$xferWrapper;commonPath=$xferCommon;wrapperSha256=(Get-FileSha256FromBytes $xferWrapper);commonSha256=(Get-FileSha256FromBytes $xferCommon);markerAuthorityContractVersion=1};if($TransferName){$contract.transferName=$TransferName};return $contract}
  function Invoke-XferCommand([ValidateSet('handshake','prepare-transfer')][string]$Mode,$Contract){$json=$Contract|ConvertTo-Json -Compress;$json64=[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($json));$savedPreference=$ErrorActionPreference;$ErrorActionPreference='Continue';$encoded=& node (Join-Path $repo 'scripts\ci\build-windows-remote-command.cjs') "$Mode-base64" $json64 2>$null;$buildExit=$LASTEXITCODE;$ErrorActionPreference=$savedPreference;if($buildExit -ne 0){throw 'XFER command build failed'};$script=[Text.Encoding]::Unicode.GetString([Convert]::FromBase64String($encoded));return @(& ([scriptblock]::Create($script)) 2>&1)}
  function Assert-XferReject([scriptblock]$Action,[string]$Id){$rejected=$false;try{&$Action|Out-Null}catch{$rejected=$true};if(-not $rejected){throw "$Id expected fail-closed rejection"}}
  function Copy-XferMarker{$xferMarker|ConvertTo-Json -Depth 10|ConvertFrom-Json}
  Write-XferMarker
  if(Test-Path $xferEntry){throw 'XFER-P1 current must be absent'}
  if((Invoke-XferCommand handshake (Get-XferContract)) -notcontains 'BAOGIANG_HANDSHAKE_PASS'){throw 'XFER-P1 valid no-current handshake failed'}
  $p2=Copy-XferMarker;$p2|Add-Member unknownProperty 'reject';Write-XferMarker $p2;Assert-XferReject {Invoke-XferCommand handshake (Get-XferContract)} 'XFER-P2'
  ...
  Write-XferMarker
  foreach($missingName in @('incoming','shared','backups')){$xferMissing=Join-Path $xferRoot $missingName;if($missingName -eq 'shared'){continue};Remove-Item $xferMissing;Assert-XferReject {Invoke-XferCommand handshake (Get-XferContract)} "XFER-P8-missing-$missingName";New-Item -ItemType Directory $xferMissing|Out-Null}
  $xferReparseTarget=Join-Path $temp 'xfer-required-reparse-target';New-Item -ItemType Directory $xferReparseTarget|Out-Null;$xferBackups=Join-Path $xferRoot 'backups';Remove-Item $xferBackups;New-Item -ItemType Junction -Path $xferBackups -Target $xferReparseTarget|Out-Null;Assert-XferReject {Invoke-XferCommand handshake (Get-XferContract)} 'XFER-P8-reparse';[IO.Directory]::Delete($xferBackups);New-Item -ItemType Directory $xferBackups|Out-Null
  $sentinel=Join-Path $temp 'tampered-common-executed';$originalCommon=[IO.File]::ReadAllBytes($xferCommon);[IO.File]::WriteAllText($xferCommon,"New-Item -ItemType File -Path '$sentinel'|Out-Null`n",[Text.UTF8Encoding]::new($false));$trusted=Get-XferContract;$trusted.commonSha256=([BitConverter]::ToString([Security.Cryptography.SHA256]::Create().ComputeHash($originalCommon))).Replace('-','').ToLowerInvariant();Assert-XferReject {Invoke-XferCommand handshake $trusted} 'XFER-P9';if(Test-Path $sentinel){throw 'XFER-P9 tampered common executed before hash rejection'};[IO.File]::WriteAllBytes($xferCommon,$originalCommon)
  $originalWrapper=[IO.File]::ReadAllBytes($xferWrapper);$trusted=Get-XferContract;[IO.File]::WriteAllText($xferWrapper,'tampered',[Text.UTF8Encoding]::new($false));Assert-XferReject {Invoke-XferCommand handshake $trusted} 'XFER-P10';[IO.File]::WriteAllBytes($xferWrapper,$originalWrapper)
  $oldCommon="function Read-DeploymentIdentity { return [pscustomobject]@{ok=`$true} }";[IO.File]::WriteAllText($xferCommon,$oldCommon,[Text.UTF8Encoding]::new($false));$oldContract=Get-XferContract;Remove-Item Function:\Get-DeploymentMarkerAuthorityContractVersion -Force;Assert-XferReject {Invoke-XferCommand handshake $oldContract} 'XFER-P11';[IO.File]::WriteAllBytes($xferCommon,$originalCommon);Write-XferMarker
  if((Invoke-XferCommand handshake (Get-XferContract)) -notcontains 'BAOGIANG_HANDSHAKE_PASS'){throw 'XFER-P12 valid version 1 failed'}
  $before=@(Get-ChildItem $xferRoot -Recurse -Force|ForEach-Object{$_.FullName});Invoke-XferCommand handshake (Get-XferContract)|Out-Null;$after=@(Get-ChildItem $xferRoot -Recurse -Force|ForEach-Object{$_.FullName});if(Compare-Object $before $after){throw 'XFER-P13 read-only handshake changed tree'}
  $transferName="control-1-1-$xferSha";Invoke-XferCommand handshake (Get-XferContract)|Out-Null;$invalid=Copy-XferMarker;$invalid.systemId='changed';Write-XferMarker $invalid;Assert-XferReject {Invoke-XferCommand prepare-transfer (Get-XferContract $transferName)} 'XFER-P14';if(Test-Path (Join-Path $xferRoot "incoming\$transferName")){throw 'XFER-P14 created transfer after marker change'}
  Write-XferMarker;if((Invoke-XferCommand prepare-transfer (Get-XferContract $transferName)) -notcontains 'BAOGIANG_PREPARE_TRANSFER_PASS'){throw 'XFER-P15 prepare failed'}
  foreach($bad in @('..','control/1','C:\escape',"control-1-1-$('A'*40)",'control-x-1-'+$xferSha)){Assert-XferReject {Invoke-XferCommand prepare-transfer (Get-XferContract $bad)} 'XFER-P16'}
  Assert-XferReject {Invoke-XferCommand prepare-transfer (Get-XferContract $transferName)} 'XFER-P17'
  $ancestorTarget=Join-Path $temp 'ancestor-target';New-Item -ItemType Directory $ancestorTarget|Out-Null;$ancestorRoot=Join-Path $ancestorTarget 'baogiang';New-Item -ItemType Directory $ancestorRoot|Out-Null;$bridge=Join-Path $xferRoot 'bridge';New-Item -ItemType Junction -Path $bridge -Target $ancestorTarget|Out-Null;$routedRoot=Join-Path $bridge 'baogiang';Assert-XferReject {Assert-DedicatedRoot $routedRoot} 'XFER-P18-root';Assert-XferReject {Read-DeploymentIdentity -Root $routedRoot -ServiceKind scheduled-task -ServiceName BaoGiangBackend -EnvFile $xferEnv -StartupWrapper $xferWrapper -ExpectedEntryPoint $xferEntry} 'XFER-P18-identity';$routedContract=Get-XferContract "control-3-1-$xferSha";$routedContract.root=$routedRoot;$routedContract.startupWrapper=Join-Path $routedRoot "shared\startup-bundles\$xferSha\start-baogiang-api.ps1";$routedContract.wrapperPath=$routedContract.startupWrapper;$routedContract.commonPath=Join-Path $routedRoot "shared\startup-bundles\$xferSha\deployment-common.ps1";Assert-XferReject {Invoke-XferCommand prepare-transfer $routedContract} 'XFER-P18-prepare';if(Test-Path (Join-Path $routedRoot "incoming\control-3-1-$xferSha")){throw 'XFER-P18 created transfer behind junction'}
  $plannedRouted=Join-Path $bridge 'planned-missing';Assert-XferReject {Assert-DedicatedRoot $plannedRouted} 'XFER-P19-root';Assert-XferReject {& (Join-Path $repo 'scripts\deploy\windows\production-root-acl-plan.ps1') -Root $plannedRouted -DeploymentIdentity fixture -ApiRuntimeIdentity fixture -WebRuntimeIdentity fixture -EnvFile $xferEnv -StartupWrapper $xferWrapper -ReportPath (Join-Path $temp 'p19-report.json')} 'XFER-P19-plan';[IO.Directory]::Delete($bridge);$safeParent=Join-Path $temp 'safe-parent';New-Item -ItemType Directory $safeParent|Out-Null;if((Assert-DedicatedRoot (Join-Path $safeParent 'planned-missing')) -notmatch 'planned-missing$'){throw 'XFER-P20 safe missing root rejected'}
  $cleanupTarget=Join-Path $temp 'cleanup-sentinel-target';New-Item -ItemType Directory $cleanupTarget|Out-Null;$cleanupSentinel=Join-Path $cleanupTarget 'sentinel.txt';[IO.File]::WriteAllText($cleanupSentinel,'safe');$cleanupName="control-2-1-$xferSha";$cleanupLink=Join-Path $xferRoot "incoming\$cleanupName";New-Item -ItemType Junction -Path $cleanupLink -Target $cleanupTarget|Out-Null;$cleanupEncoded=&node (Join-Path $repo 'scripts\ci\build-windows-remote-command.cjs') cleanup $xferRoot $cleanupName;$cleanupScript=[Text.Encoding]::Unicode.GetString([Convert]::FromBase64String($cleanupEncoded));Assert-XferReject {&([scriptblock]::Create($cleanupScript))} 'XFER-P21';if(-not(Test-Path $cleanupSentinel)){throw 'XFER-P21 cleanup traversed junction'}
  $gitFixture=Join-Path $temp 'xfer-git';New-Item -ItemType Directory (Join-Path $gitFixture 'scripts\deploy\windows') -Force|Out-Null;Copy-Item (Join-Path $repo 'scripts\deploy\windows\start-baogiang-api.ps1') (Join-Path $gitFixture 'scripts\deploy\windows\start-baogiang-api.ps1');Copy-Item (Join-Path $repo 'scripts\deploy\windows\deployment-common.ps1') (Join-Path $gitFixture 'scripts\deploy\windows\deployment-common.ps1');&git -C $gitFixture init -b main|Out-Null;&git -C $gitFixture config user.email fixture@example.test;&git -C $gitFixture config user.name Fixture;&git -C $gitFixture add -- .;&git -C $gitFixture commit -m fixture|Out-Null;$gitSha=(&git -C $gitFixture rev-parse HEAD).Trim();$evidenceInput=@{repositoryRoot=$gitFixture;canonicalMainRef='main';root='C:\baogiang';startupWrapper="C:\baogiang\shared\startup-bundles\$gitSha\start-baogiang-api.ps1"}|ConvertTo-Json -Compress;$evidenceInput64=[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($evidenceInput));$evidenceA=&node (Join-Path $repo 'scripts\ci\resolve-startup-bundle-handshake-evidence.cjs') --base64 $evidenceInput64|ConvertFrom-Json;[IO.File]::AppendAllText((Join-Path $gitFixture 'scripts\deploy\windows\deployment-common.ps1'),'dirty');$evidenceB=&node (Join-Path $repo 'scripts\ci\resolve-startup-bundle-handshake-evidence.cjs') --base64 $evidenceInput64|ConvertFrom-Json;if($evidenceA.commonGitBlobOid -cne $evidenceB.commonGitBlobOid -or $evidenceA.commonSha256 -cne $evidenceB.commonSha256){throw 'XFER-P22 dirty working tree affected Git evidence'}
  foreach($badWrapper in @("C:\baogiang\shared\startup-bundles\$('A'*40)\start-baogiang-api.ps1","C:\baogiang\outside\$gitSha\start-baogiang-api.ps1","C:\baogiang\shared\startup-bundles\$gitSha\wrong.ps1")){$badInput=@{repositoryRoot=$gitFixture;canonicalMainRef='main';root='C:\baogiang';startupWrapper=$badWrapper}|ConvertTo-Json -Compress;$badInput64=[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($badInput));$savedPreference=$ErrorActionPreference;$ErrorActionPreference='Continue';&node (Join-Path $repo 'scripts\ci\resolve-startup-bundle-handshake-evidence.cjs') --base64 $badInput64 2>$null|Out-Null;$badExit=$LASTEXITCODE;$ErrorActionPreference=$savedPreference;if($badExit -eq 0){throw 'XFER-P23 invalid layout accepted'}}
  $gitTree=(&git -C $gitFixture rev-parse 'HEAD^{tree}').Trim();$sideSha=((&git -C $gitFixture commit-tree $gitTree -m side) | Select-Object -Last 1).Trim();$sideInput=@{repositoryRoot=$gitFixture;canonicalMainRef='main';root='C:\baogiang';startupWrapper="C:\baogiang\shared\startup-bundles\$sideSha\start-baogiang-api.ps1"}|ConvertTo-Json -Compress;$sideInput64=[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($sideInput));$savedPreference=$ErrorActionPreference;$ErrorActionPreference='Continue';&node (Join-Path $repo 'scripts\ci\resolve-startup-bundle-handshake-evidence.cjs') --base64 $sideInput64 2>$null|Out-Null;$sideExit=$LASTEXITCODE;$ErrorActionPreference=$savedPreference;if($sideExit -eq 0){throw 'XFER-P23 non-first-parent commit accepted'}
  $p24Json=(Get-XferContract)|ConvertTo-Json -Compress;$p24Json64=[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($p24Json));$p24Encoded=&node (Join-Path $repo 'scripts\ci\build-windows-remote-command.cjs') handshake-base64 $p24Json64;$p24Script=[Text.Encoding]::Unicode.GetString([Convert]::FromBase64String($p24Encoded));if($p24Script -match '(?i)\.\s+\$common(?:;|$)|&\s*\$common(?:;|$)|powershell(?:\.exe)?[^;]*-File[^;]*\$common|ReadAllBytes\(\$common\)|Get-Content[^;]*\$common'){throw 'XFER-P24 generated authority reopens or executes common by path'};if(([regex]::Matches($p24Script,'\[IO\.FileStream\]::new\(\$common')).Count -ne 1 -or $p24Script -notmatch '\[Text\.UTF8Encoding\]::new\(\$false,\$true\)' -or $p24Script -notmatch '\[ScriptBlock\]::Create\(\$commonText\)' -or $p24Script -notmatch '\. \$trustedCommon'){throw 'XFER-P24 verified in-memory common execution structure is missing'}
