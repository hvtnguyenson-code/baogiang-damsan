...
if ($ServiceKind -eq 'scheduled-task') {
  ...
  $health = Invoke-ScheduledTaskRollbackLifecycle -Context $rollbackContext -Restart { param($context) & (Join-Path $PSScriptRoot 'restart-baogiang-api.ps1') -ServiceKind $ServiceKind -ServiceName $ServiceName -NodeExe $NodeExe -Root $Root -EnvFile $EnvFile -StartupWrapper $StartupWrapper -ExpectedEntryPoint $ExpectedEntryPoint -ExpectedBaseUrl $ExpectedBaseUrl -AllowScheduledTaskActivation:$AllowScheduledTaskActivation } -Health { param($context) & (Join-Path $PSScriptRoot 'test-production-health.ps1') -BaseUrl $ExpectedBaseUrl -ExpectedApiPort 3100 } -SafeStop { param($context) Stop-ExactBaoGiangRuntime -Marker $marker -ServiceKind $ServiceKind -ServiceName $ServiceName | Out-Null }
