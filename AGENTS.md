...
1. Verify repository path, branch, exact HEAD, canonical `origin/main`, `git status -sb`, remote divergence, and untracked files.
...
10. After merge, the task is `MERGED_AWAITING_DOC_SYNC`, not `CLOSED`. Record exact merge/main SHA and authoritative post-merge CI through the mandatory documentation-sync procedure before any dependent major task starts.
...
- exact post-merge evidence must be recorded;
