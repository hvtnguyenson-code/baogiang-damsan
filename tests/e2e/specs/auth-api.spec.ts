...
test('auth cookie flow, first password change, logout, and CSRF rejection', async () => {
  ...
  const login = await api.post('/api/auth/login', {
    data: { username: 'e2e-api-admin', password: 'E2eApiBootstrapPassword9' },
  });
  ...
  const wrongOrigin = await api.post('/api/auth/change-password', {
    ...
    data: { currentPassword: 'E2eApiBootstrapPassword9', newPassword: 'E2eApiReplacementPassword8' },
  });
  ...
  const oldPassword = await api.post('/api/auth/login', { data: { username: 'e2e-api-admin', password: 'E2eApiBootstrapPassword9' } });
  expect(oldPassword.status()).toBe(401);
  const newPassword = await api.post('/api/auth/login', { data: { username: 'e2e-api-admin', password: 'E2eApiReplacementPassword8' } });
